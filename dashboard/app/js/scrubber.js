...
function scrubber_add_button_click()
{
    $('#dealership_id').val('');
    $('#job_title').val('');
    $('#contact_name').val('');
    ...
    $('#website').val('');
    $('#dealership_name').val('');
    $('#status option[value="Open"]').attr('selected', 'selected');
...
function scrubber_push_button_click(sender)
{
    ...
    var result = confirm('Do you really want to push it to Closer?');
    
    if(result)
    {
        show_progress('Pushing to Closer . . . ');
    ...
    $('.scrubber-editor').dialog({
        ...
        buttons: {
            "Save": function() {
                ...
                url = 'ajax.php?act=save_scrubber';
