...
$campaign_parts = explode('_', $campaign);
$campType = count($campaign_parts) - 2;
$stock_type = $campaign_parts[1];
...
switch ($campType) {
	case 1:
		$campaign_name = "make";
		$vars = ['make' => $selectedMake];
		break;
	case 2:
		$campaign_name = "make_model";
		$vars = ['make' => $selectedMake, 'model' => $selectedModel];
		break;
	case 3:
		$campaign_name = "make_model_year";
		$vars = ['make' => $selectedMake, 'model' => $selectedModel, 'year' => $selectedYear];
		break;
	case 4:
		$campaign_name = "make_model_year_trim";
		$vars = ['make' => $selectedMake, 'model' => $selectedModel, 'year' => $selectedYear, 'trim' => $selectedTrim];
		break;
}


// $default_settings = AdwordsController::$defaultSettings['rsa'][$stock_type][$campaign_name];

$adwordsController = new AdwordsController($dealership);
$adwordsController->setType($stock_type);
$default_settings = $adwordsController->getRsaSetting($campaign_name, $vars);
...
if (empty($selectedMake) && empty($selectedModel) && empty($selectedYear) && empty($selectedTrim)) {
	$formatter = function ($positions) {
		return function ($v, $k) use ($positions) {
			return array_map(function ($_v) use ($k, $positions) {
				return [
					'text'	=> $_v,
	...
	$setting['headings'] = collect($default_settings['titles'])->map($formatter(['h', 'h1', 'h2', 'h3']))->flatten(1)->toArray();

	$setting['descriptions'] = collect($default_settings['descriptions'])->map($formatter(['d', 'd1', 'd2']))->flatten(1)->toArray();
}
																...
																<div class="row">
																	...
																	<div class="col-md-3">
																		<select class="form-control" name='headings[<?= $i ?>][position]'>
																			<?= implode('', option_maker(['h1' => "Heading 1", 'h2' => "Heading 2", 'h3' => "Heading 3", 'h' => "Any"], $heading['position'])) ?>
																		</select>
																	</div>
																</div>
															</td>
																...
																<div class="row">
																	...
																	<div class="col-md-3">
																		<select class="form-control" name='descriptions[<?= $i ?>][position]'>
																			<?= implode('', option_maker(['d1' => "Description 1", 'd2' => "Description 2", 'd' => "Any"], $description['position'])) ?>
																		</select>
																	</div>
																</div>
															</td>
							...
							<div class="row">
								...
								<div class="col-md-3">
									<select class="form-control" name='headings[{{id}}][position]'>
										<?= implode('', option_maker(['h1' => "Heading 1", 'h2' => "Heading 2", 'h3' => "Heading 3", 'h' => "Any"], 'h')) ?>
									</select>
								</div>
							</div>
						</td>
								...
								<div class="row">
									...
									<div class="col-md-3">
										<select class="form-control" name='descriptions[{{id}}][position]'>
											<?= implode('', option_maker(['d1' => "Description 1", 'd2' => "Description 2", 'd' => "Any"], 'd')) ?>
										</select>
									</div>
								</div>
							</td>
