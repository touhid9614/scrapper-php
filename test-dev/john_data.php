...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $dealer_data[$row["dealership"]][] = $row["city"];
    $dealer_data[$row["dealership"]][] = $row["state"];
    $dealer_data[$row["dealership"]][] = $row["post_code"];
    $dealer_data[$row["dealership"]][] = $row["address"];
    $dealer_data[$row["dealership"]][] = $row["phone"];
}

$fields          = ['stock_number', 'vin', 'stock_type', 'title', 'year', 'make', 'model', 'trim', 'msrp', 'price', 'body_style', 'engine', 'transmission', 'fuel_type', 'drivetrain', 'exterior_color', 'interior_color', 'kilometres', 'all_images', 'description', 'url'];
$timestamps      = ['arrival_date', 'updated_at'];
$address_fields  = ['city', 'state', 'post_code', 'address', 'phone'];
$extended_fields = array_merge(['dealership'], $fields, $timestamps, $address_fields);
...
foreach ($tables as $table_name) {
    ...
    while ($row_data = mysqli_fetch_assoc($scrap)) {
        foreach ($fields as $current_field) {
            $current_row_values[] = $row_data[$current_field];
        }
