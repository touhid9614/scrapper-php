...
$selectedFeatures = [
    ...
    foreach ($allactive_dealers as $cron_name => $dealer_data) 
    {
         ...
         */
        $unsoldCarUrls = [];
        $get_allunsoldcar = DbConnect::get_instance()->query("SELECT url, arrival_date, updated_at FROM {$cron_name}_scrapped_data WHERE deleted='0'");
        while ($car = mysqli_fetch_assoc($get_allunsoldcar)) {
        ...
        foreach ($data_files as $file_name) {
            ...
            foreach ($file_content as $session_id => $session_data) {
                foreach ($session_data['_views'] as $view_id => $view_data) {
                     ...
                     */
                    $car_url = "";
                    foreach ($unsoldCarUrls as $key_url => $car_val) {
                    ...
                    if (!isset($featuresData[$view_data['url']])) {
                        foreach ($selectedFeatures as $key => $value) {
                            if($key == 'time_on_inventory') {
                                $featuresData[$view_data['url']][$key] = $unsoldCarUrls[$view_data['url']]['days'];
                            } else {
         ...
         */
         if(count($featuresData)) {
             foreach ($featuresData as $url => $urldata) {
                 ...
                 $salematrixQuery = DbConnect::get_instance()->query("SELECT dealership FROM salematrix_featuredata_unsold WHERE dealership='{$cron_name}' AND url = '$url'");
                 if($salematrixQuery->num_rows) { 
                        $updateStr = "UPDATE salematrix_featuredata_unsold SET ";
                     ...
                     DbConnect::get_instance()->query("INSERT INTO salematrix_featuredata_unsold VALUES('{$cron_name}', '{$url}', {$urldata},'{$current_time}')");  
                 }      
             }
         }
