...
function insert_update_feature_data_all($cron_name, $featuresData)
{
    if (count($featuresData)) {
        ...
        foreach ($featuresData as $url => $urlData) {
            $dealerExist = url_exist_in_table('salematrix_featuredata_unsold', $cron_name, $url);
            ...
            if ($dealerExist->fetch()) {
                ...
                ConnectMysql::get_instance()->exeInsertOrUpdate('salematrix_featuredata_unsold', $urlData, $where);
                ...
                $urlData['url'] = $url;
                $urlData['last_updated'] = $current_time;
                ConnectMysql::get_instance()->exeInsertOrUpdate('salematrix_featuredata_unsold', $urlData);
...
foreach ($all_active_dealers as $cron_name => $website) {
     ...
     */
    $unSoldCarUrls  = get_all_unsold_car($cron_name);
    $featuresData   = [];
    ...
    foreach ($pageViews as $pageview) {
         ...
         */
        $car_url = url_exit($unSoldCarUrls, $pageview['url']);
        ...
        */
        if (!isset($featuresData[$pageview['url']])) {
            ...
            $featuresData[$pageview['url']]['time_on_inventory'] = $unSoldCarUrls[$car_url]['days'];
        }
    ...
    /*
     * Insert/Update mean and sd data into sale matrix feature data unsold table
     */
