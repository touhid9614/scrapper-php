...
function eval_dealership($CurrentConfig, $cron_name, $mutex)
{
    ...
    if (!$cron_config) {return array('error' => array('code' => 404, 'message' => 'Dealership is not present'));}
    ...
    if ($profileId) {
        ...
        $analyticsReport_l = get_analytics_report($analytics, $profileId, $analyticsStartDate_l, $analyticsEndDate_l, $metrics = array('ga:avgTimeOnPage', 'ga:bounceRate'), array('ga:campaign'), $filters, 8766);
    ...
    $to_return['inventory'] = create_eval_report($cron_name, $total_days, $days_past, $days_remaining, $budget, $spent, $on15, $range, $fb_budget);
    ...
    $to_return['inventory']['customer_id']     = $cron_config['customer_id'];
    $to_return['inventory']['profile_id']      = $profileId;
    $to_return['inventory']['y_adb']           = $yesterday_spent - $to_return['adjustment'];
    $to_return['inventory']['yesterday_spent'] = $yesterday_spent;
    $to_return['inventory']['today_spent']     = $today_spent;
    $to_return['inventory']['daily_average']   = $weekly_average;
    $to_return['inventory']['clicks']          = $calced['clicks'] - $calced_l['clicks'];
    $to_return['inventory']['impressions']     = $calced['impressions'] - $calced_l['impressions'];
    ...
    $to_return['inventory']['projected_clicks']         = $projected_clicks;
    $to_return['inventory']['projected_impressions']    = $projected_impressions;
    $to_return['inventory']['total_clicks']             = $calced['clicks'];
    $to_return['inventory']['bounce_rate']              = $bounceRate;
    $to_return['inventory']['bounce_rate_pp']           = $bounceRate_l;
    $to_return['inventory']['advert_type']              = 'regular';
    $to_return['inventory']['custom']                   = $custom;
    $to_return['inventory']['cost_per_engaged_user']    = $bounceRate !== null ? ($calced['clicks'] > 0 ? ($calced['clicks'] - ($calced['clicks'] * $bounceRate) > 0 ? (round($spent / ($calced['clicks'] - ($calced['clicks'] * $bounceRate)), 2)) : 'Inf') : 0) : 'N/A';
    $to_return['inventory']['cost_per_engaged_user_pp'] = $bounceRate_l !== null ? ($calced_l['clicks'] > 0 ? ($calced_l['clicks'] - ($calced_l['clicks'] * $bounceRate_l) > 0 ? (round($spent_l / ($calced_l['clicks'] - ($calced_l['clicks'] * $bounceRate_l)), 2)) : 'Inf') : 0) : 'N/A';
    $to_return['inventory']['campaigns']                = compile_campaigns($analyticsReport, $analyticsReport_l, $calced['campaigns'], $calced_l['campaigns'], $calced_y['campaigns'], $calced_t['campaigns']);
...
function update_budget($CurrentConfig, $cron_name)
{
    ...
    for ($i = 0; $i < count($changed_budgets); $i++) {
...
function create_eval_report($cron_name, $total_days, $days_past, $days_remaining, $budget, $spent, $on15, $ranged, $fb_budget = 0)
{
    ...
    if ($days_remaining <= 0) {$adjustment = round($budget - $spent, 2);} else { $adjustment = round(($budget - $spent) / $days_remaining, 2);}
...
function compile_campaigns($analyticsReport, $analyticsReport_l, $campaigns, $campaigns_lastmonth, $campaigns_yesterday, $campaigns_today)
{
    foreach ($campaigns as $campaign_name => $campaign) {
        ...
        if ($analyticsReport) {$bounceRate = 0;}
        if ($analyticsReport_l) {$bounceRate_l = 0;}
