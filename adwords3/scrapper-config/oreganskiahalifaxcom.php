...
$scrapper_configs["oreganskiahalifaxcom"] = array(
    'entry_points' => array(
        'new' => 'https://oserv2.oreganscdn.com/api/vehicle-inventory-search/?app.widgetProfile.id=20753cd7-f61c-4de6-9b83-7bf2457d9a4c.searchResults.search&search.vehicle-inventory-type-ids=1&do-search=1&search.results-offset=0&search.results-limit=50&app.referrer=https://www.oreganskiahalifax.com',
        'used' => 'https://www.oreganskiahalifax.com/inventory/pre-owned/?search.start-index=0&search=1&do-search=1&advanced=0&system.view-type.code=ajax',
    ),
    'vdp_url_regex' => '/(?:pre-owned|new|used-cars)\/.*[0-9]{4}-/',
    'new' => array(
        'vdp_url_regex' => '/\/new\/New-[0-9]{4}-/',
        'use-proxy' => true,
        ...
        'custom_data_capture' => function($url, $data) {
            ...
            foreach ($objects->oreg->wuvs->results->vehicles as $obj) {
                ...
                $car_data = array(
                    ...
                    'url' => "https://www.oreganskiahalifax.com/inventory/pre-owned/{$obj->year}-{$obj->vehicleMake->name}-{$obj->vehicleMakeModel->name}-"
...
add_filter('filter_oreganskiahalifaxcom_car_data', 'filter_oreganskiahalifaxcom_car_data');
add_filter("filter_oreganskiahalifaxcom_next_page", "filter_oreganskiahalifaxcom_next_page", 10, 2);

function filter_oreganskiahalifaxcom_car_data($car_data) {

    if ($car_data['make'] != 'Kia') {
        return [];
    }
    ...
    if ($car_data['stock_type'] == 'new') {
        ...
        $car_data['url'] = str_replace('https://', 'https://www.oreganskiahalifax.com', $car_data['url']);
    ...
    if (isset($car_data['vin']) && $car_data['vin']) {
        ...
        $in_cookies = '';
        $out_cookies = '';
        $response_data = HttpPost($api_url, $post_data, $in_cookies, $out_cookies, false, true, 'application/x-www-form-urlencoded', ['origin' => 'https://www.oreganskiahalifax.com']);
...
function filter_oreganskiahalifaxcom_next_page($next, $current_page) {
