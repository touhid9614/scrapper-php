<?php
    ...
    $scrapper_configs['trianglenissan'] = array(
        'entry_points' => array(
            'new'   => 'http://www.trianglenissan.com/nissan.aspx',
            'used'  => 'http://www.trianglenissan.com/used-cars.aspx'
        ),
        ...
        'data_capture_regx' => array(
            ...
            'year'              => '/ebiz-vdp-title"><a href="[^"]+">(?<year>[0-9]{4})\s*(?<make>[^\s]+)\s*(?<model>[^<]*)/',
            'make'              => '/ebiz-vdp-title"><a href="[^"]+">(?<year>[0-9]{4})\s*(?<make>[^\s]+)\s*(?<model>[^<]*)/',
            'model'             => '/ebiz-vdp-title"><a href="[^"]+">(?<year>[0-9]{4})\s*(?<make>[^\s]+)\s*(?<model>[^<]*)/',
