...
$scrapper_configs["johnsonhyundaicom"] = array(
    'entry_points'        => array(
        'used' => 'https://www.johnsonhyundaiofcary.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
        'new'  => 'https://www.johnsonhyundaiofcary.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1',
    ),
    ...
    'custom_data_capture' => function ($url, $data) {
        ...
        foreach ($objects->pageInfo->trackingData as $obj) {
            $car_data = array(
                ...
                'url'            => "https://www.johnsonhyundaiofcary.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}" . ".htm",
...
add_filter("filter_johnsonhyundaicom_next_page", "filter_johnsonhyundaicom_next_page", 10, 2);
function filter_johnsonhyundaicom_next_page($next, $current_page)
{
    $start_tag = 'start=';
    $end_tag   = '&';
