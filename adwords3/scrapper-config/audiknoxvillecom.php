...
$scrapper_configs["audiknoxvillecom"] = array( 
	 "entry_points" => array(  
        'new' => 'https://www.audiknoxville.com/new-inventory/index.htm',
    ...
    'data_capture_regx' => array(
        ...
        'price' => '/Harper Price[^>]+>:[^>]+>[^>]+>[^>]+>(?<price>[^<]+)/', ///dealer price///
...
add_filter("filter_audiknoxvillecom_field_price", "filter_audiknoxvillecom_field_price", 10, 3);
add_filter("filter_audiknoxvillecom_field_images", "filter_audiknoxvillecom_field_images");
  

function filter_audiknoxvillecom_field_images($im_urls) {
   $retval = [];
...
function filter_audiknoxvillecom_field_price($price, $car_data, $spltd_data) {
    $prices = [];
    ...
    $internet_regex = '/Internet Price[^>]+>:[^>]+>[^>]+>[^>]+>(?<price>[^<]+)/';  //internet price//
