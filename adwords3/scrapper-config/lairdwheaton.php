...
$scrapper_configs['lairdwheaton'] = array(
   'entry_points' => array(
        'used' => 'https://www.lairdwheaton.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
        'new' => 'https://www.lairdwheaton.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1'
    ),
            ...
            foreach ($objects->pageInfo->trackingData as $obj) {
                ...
                $car_data = array(
                   ...
                   // 'images'            => array_merge($obj->photos->user, $obj->photos->stock),    
                    'url' => "https://www.lairdwheaton.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
                                
                );
                    if(preg_match("/Bolt/", $car_data['model'])){
                        continue;
                    }
        ...
        add_filter("filter_lairdwheaton_field_images", "filter_lairdwheaton_field_images");

function filter_lairdwheaton_field_images($im_urls)
    {
        $retval = [];
 ...
 add_filter("filter_lairdwheaton_next_page", "filter_lairdwheaton_next_page", 10, 2);       
 function filter_lairdwheaton_next_page($next, $current_page) {
