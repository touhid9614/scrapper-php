...
$scrapper_configs['cheyennegm'] = array(
    'entry_points' => array(
        'new' => 'https://www.cheyennegm.ca/VehicleSearchResults?search=new',
        'used' => 'https://www.cheyennegm.ca/VehicleSearchResults?search=preowned',
    ),
...
add_filter("filter_cheyennegm_next_page", "filter_cheyennegm_next_page", 10, 2);

add_filter("filter_cheyennegm_field_images", "filter_cheyennegm_field_images");

function filter_cheyennegm_next_page($next, $current_page) {
    slecho("Filtering Next url");
    $car_type = explode('=', $current_page);
    return urlCombine($next, "?search={$car_type[count($car_type) - 1]}");
}

function filter_cheyennegm_field_images($im_urls) {
    unset($im_urls[0]);
