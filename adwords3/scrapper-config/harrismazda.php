...
global $scrapper_configs;
 $scrapper_configs["harrismazda"] = array( 
	 'entry_points' => array(
            'new'    => 'https://www.harrismazda.ca/new-vehicles/',
            'used'   => 'https://www.harrismazda.ca/used-vehicles/'
        ),
        'use-proxy' => true,
        //'proxy-area'        => 'FL',
    ...
    add_filter('filter_harrismazda_post_data', 'filter_harrismazda_post_data', 10, 3);
    add_filter('filter_harrismazda_data', 'filter_harrismazda_data');
    add_filter("filter_harrismazda_field_images", "filter_harrismazda_field_images");
    
    $harrismazda_nonce = '';
    
    function filter_harrismazda_post_data($post_data, $stock_type, $data)
    {
        global $harrismazda_nonce;
        if($post_data == '')
        {
            $post_data = "page=1";
        }
        ...
        if($nonce && isset($nonce)) { $harrismazda_nonce=$nonce; }
        slecho("global ajax_nonce : ".$harrismazda_nonce);
        $post_id = 4;
        ...
        return "action=im_ajax_call&perform=get_results&$post_data&_nonce=$harrismazda_nonce&_post_id=$post_id&_referer=$referer";
    }

    function filter_harrismazda_data($data)
    {
        if($data)
        {
            if(isJSON($data)){
    ...
    function filter_harrismazda_field_images($im_urls)
    {
