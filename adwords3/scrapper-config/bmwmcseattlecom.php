...
$scrapper_configs["bmwmcseattlecom"] = array( 
	"entry_points" => array(
	    'used' => 'https://www.bmwmcseattle.com/--inventory?condition=pre-owned&pg=1',
            'new' => 'https://www.bmwmcseattle.com/--inventory?condition=new&pg=1',
    ...
    'data_capture_regx_full' => array(
      ...
      //  'description' => '/<meta name="description" content="(?<description>[^"]+)/',
       //  'engine' => '/Engine<\/label>\s*[^>]+>(?<engine>[^\<]+)/',
         'kilometres' => '/Odometer<\/label>\s*[^>]+>(?<kilometres>[^<]+)/',
...
add_filter("filter_bmwmcseattlecom_next_page", "filter_bmwmcseattlecom_next_page", 10, 2);
add_filter("filter_bmwmcseattlecom_field_images", "filter_bmwmcseattlecom_field_images");

add_filter('filter_bmwmcseattlecom_car_data', 'filter_bmwmcseattlecom_car_data');

function filter_bmwmcseattlecom_car_data($car_data) {
    ...
    if(strlen($car_data['model'])>20){
        $car_data['model']=preg_replace('/\s[A-Za-z0-9 -]+/', '', $car_data['model']);
    }
     
    return $car_data;
}

function filter_bmwmcseattlecom_next_page($next, $current_page) {
...
add_filter("filter_bmwmcseattlecom_field_images", "filter_bmwmcseattlecom_field_images");
function filter_bmwmcseattlecom_field_images($im_urls)
{
    $retval = [];

    foreach($im_urls as $img)
    {
        $retval[] = str_replace(["&#x2F;","https://www.bmwmcseattle.com/"], ["/",""], $img);
    }
