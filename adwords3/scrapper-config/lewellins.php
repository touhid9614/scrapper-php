...
$scrapper_configs["lewellins"] = array(
    'entry_points' => array(
        'new' => 'https://www.lewellins.net/search/inventory/usage/New',
        'used' => 'https://www.lewellins.net/search/inventory/usage/Used'
    ),
...
add_filter("filter_lewellins_field_images", "filter_lewellins_field_images");

function filter_lewellins_field_images($im_urls) {
    $retval = array();
    // slecho(implode('|', $im_urls));
    foreach ($im_urls as $im_url) {
        $retval[] = str_replace(['http://www.lewellins.net/inventory/', '%3a', '%2f','http://www.lewellins.net/new-models/'], ['', ':', '/',''], rawurldecode($im_url));
    }
