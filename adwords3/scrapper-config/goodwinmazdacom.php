...
$scrapper_configs["goodwinmazdacom"] = array( 
	"entry_points" => array(

        'new' => 'https://www.goodwinmazda.com/new-inventory/index.htm',
        'used' => 'https://www.goodwinmazda.com/used-inventory/index.htm'
    ),
    'vdp_url_regex' => '/\/(?:new|certified|used|commercial-used)\/[^\/]+\/[0-9]{4}-/i',
    'use-proxy' => true,
    ...
    'data_capture_regx' => array(
        ...
        'kilometres' => '/<dt>Mileage:[^>]+>[^>]+>(?<kilometres>[^\s*]+)/',
...
add_filter("filter_goodwinmazdacom_field_price", "filter_goodwinmazdacom_field_price", 10, 3);

function filter_goodwinmazdacom_field_price($price, $car_data, $spltd_data) {
    $prices = [];
    ...
    $wholesale_regex = '/Final Price<[^>]+>:<\/span><\/span><[^>]+>(?<price>\$[0-9,]+)/';
