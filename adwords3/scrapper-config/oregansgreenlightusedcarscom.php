...
$scrapper_configs["oregansgreenlightusedcarscom"] = array(
    'entry_points' => array(
        'used' => 'https://www.oregansgreenlightusedcars.com/used-cars/?search.location.id=14&search.start-index=0&do-search=1&advanced=&system.view-type.code=ajax',
    ),
        ...
        'custom_data_capture' => function($url, $data) {
            ...
            foreach ($objects->oreg->wuvs->results->vehicles as $obj) {
                ...
                $car_data = array(
                    ...
                    'url' => "https://www.oregansgreenlightusedcars.com/used-cars/{$obj->year}-{$obj->vehicleMake->name}-{$obj->vehicleMakeModel->name}-"
...
add_filter('filter_oregansgreenlightusedcarscom_car_data', 'filter_oregansgreenlightusedcarscom_car_data');
add_filter("filter_oregansgreenlightusedcarscom_next_page", "filter_oregansgreenlightusedcarscom_next_page", 10, 2);

function filter_oregansgreenlightusedcarscom_car_data($car_data) {
    ...
    if ($car_data['stock_type'] == 'new') {
        ...
        $car_data['url'] = str_replace('https://', 'https://www.oregansgreenlightusedcars.com', $car_data['url']);
    ...
    if (isset($car_data['vin']) && $car_data['vin']) {
        ...
        $in_cookies = '';
        $out_cookies = '';
        $response_data = HttpPost($api_url, $post_data, $in_cookies, $out_cookies, false, true, 'application/x-www-form-urlencoded', ['origin' => 'https://www.oregansgreenlightusedcars.com']);
...
function filter_oregansgreenlightusedcarscom_next_page($next, $current_page) {
