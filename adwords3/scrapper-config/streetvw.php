...
$scrapper_configs["streetvw"] = array(
    "entry_points" => array(
        'new' => 'https://www.streetvw.com/new-vehicles/',
        'used' => 'https://www.streetvw.com/used-vehicles/'
    ),
    'use-proxy' => true,
    ...
    'data_capture_regx' => array(
        ...
        'kilometres' => '/class="detail-label">Mileage:<\/span>\s[^>]+>\s(?<kilometres>[^<]+)/',
...
add_filter('filter_streetvw_post_data', 'filter_streetvw_post_data', 10, 3);
add_filter('filter_streetvw_data', 'filter_streetvw_data');

$streetvw_nonce = '';

function filter_streetvw_post_data($post_data, $stock_type, $data)
{
    global $streetvw_nonce;
    if ($post_data == '') {
        $post_data = "page=1";
    }
    ...
    if ($nonce && isset($nonce)) {
        $streetvw_nonce = $nonce;
    }
    slecho("global ajax_nonce : " . $streetvw_nonce);
    $post_id = 4;
    ...
    return "action=im_ajax_call&perform=get_results&$post_data&_nonce=$streetvw_nonce&_post_id=$post_id&_referer=$referer";
}

function filter_streetvw_data($data)
{
    if ($data) {
        if (isJSON($data)) {
