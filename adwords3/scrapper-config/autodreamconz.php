...
$scrapper_configs["autodreamconz"] = array( 
	'entry_points' => array(
        'used' => 'https://www.autodream.co.nz/vehicles',
   ...
   'picture_selectors' => ['.gallery-thumb-wrapper .gallery-thumbs .thumb-item'],
    ...
    'picture_prevs' => ['.gallery-counter .icon-arrow-left'],
     'details_start_tag' => '<ul class="vehicle-list',
    ...
    'data_capture_regx_full' => array(
        ...
        'lease'      => '/<h3 class="short-desc">[^\$]+(?<lease>\$[0-9.,]+)\/Week/',
    ),
    'next_page_regx' => '/<a class="btn-next" href="(?<next>[^"]+)">><\/a>/',
    'images_regx' => '/<a style="" onclick[^<]+<img src="..\/..\/(?<img_url>[^"]+)/'
);

add_filter("filter_autodreamconz_field_images", "filter_autodreamconz_field_images");
function filter_autodreamconz_field_images($im_urls)
{
    $new_im_urls = [];
    $url_base = "https://www.autodream.co.nz";
     foreach ($im_urls as $im_url)
    {
        $new_im_url = preg_replace("/https:\/\/www.autodream.co.nz\/vehicle\/[^\/]+/", $url_base, $im_url);
        array_push($new_im_urls, $new_im_url);
    }
 if(count($new_im_urls)<=4)
            {
            return [];
            
            }
   else{ return $new_im_urls;}
}

add_filter('filter_autodreamconz_car_data', 'filter_autodreamconz_car_data');

function filter_autodreamconz_car_data($car_data) {

    if (isset($car_data['lease']) && $car_data['lease']) {
