<?php
    ...
    $scrapper_configs['audiontario'] = array(
        'entry_points' => array(
            'new'  => 'https://www.audiontario.com/new-inventory/index.htm',
           // 'used' => 'https://www.audiontario.com/used-inventory/index.htm'
        ),
        ...
        'data_capture_regx_full' => array(        
            ...
            'trim'          => '@"trim": "(?<trim>[^"]+)@',
            'price' => '/class="h1 price" >(?<price>\$[0-9,]+)<\/strong>\s*<span [^>]+>Our Price/'
        ) ,
        'next_page_regx'    => '/href="(?<next>[^"]+)"\s*rel="next"/',
        'images_regx'       => '/<a href="(?<img_url>[^"]+)"\s*class="js-link">/'
    );
    
  //  add_filter('filter_audiontario_field_images','filter_audiontario_field_images');
    add_filter("filter_audiontario_field_price", "filter_audiontario_field_price", 10, 3);
    
  /*  function filter_audiontario_field_images($im_urls)
    {
        array_push($im_urls, array_shift($im_urls));
        
          return !endsWith($im_url, '640-en_US.jpg','2981x.jpg','86671631x.jpg');
        
    }
    */
  
    function filter_audiontario_field_price($price,$car_data, $spltd_data)
     {
        $prices = [];
