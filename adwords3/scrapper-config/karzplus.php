<?php
    ...
    $scrapper_configs['karzplus'] = array(
        'entry_points' => array(
            'used'   => 'https://www.karzplus.com/used-vehicles/',      
        ),
        'use-proxy' => true,
        ...
        'init_method'       => 'GET',
        'next_method'       => 'POST',
        'picture_selectors' => ['div.gallery-thumbs div.owl-wrapper-outer div.owl-wrapper div.owl-item'],
    ...
    add_filter('filter_karzplus_post_data', 'filter_karzplus_post_data', 10, 3);
    add_filter('filter_karzplus_data', 'filter_karzplus_data');

    function filter_karzplus_post_data($post_data, $stock_type, $data)
    {
        global $karzplus_nonce;
        if($post_data == '')
        {
            $post_data = "page=1";
        }
        ...
        if($nonce && isset($nonce)) { $karzplus_nonce=$nonce; }
        slecho("global ajax_nonce : ".$karzplus_nonce);
        $post_id = 6;
        ...
        return "action=im_ajax_call&perform=get_results&$post_data&show_all_filters=false&_nonce=$karzplus_nonce&_post_id=$post_id&_referer=$referer";
    }

    function filter_karzplus_data($data)
    {
        if($data)
        {
            if(isJSON($data)){
