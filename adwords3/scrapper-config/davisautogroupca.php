...
$scrapper_configs["davisautogroupca"] = array( 
	"entry_points" => array(

		'new' => 'https://www.davisautogroup.ca/new/',
        'used' => 'https://www.davisautogroup.ca/used/',
    ...
    'data_capture_regx' => array(
        ...
        'price' => '/<span style="[^"]+"\s*itemprop="price"\s*content="[^>]+>(?<price>[^\<]+)/',
...
add_filter("filter_davisautogroupca_field_price", "filter_davisautogroupca_field_price", 10, 3);

function filter_davisautogroupca_field_price($price, $car_data, $spltd_data) {
    $prices = [];
    ...
    $old_regex = '/<span itemprop="price"[^\>]+>(?<price>[^\<]+)<\/span><\/span>/';
    $msrp_regex = '/<span itemprop="price"\s*style=""\s*content="[^>]+>(?<price>[^\<]+)/';
    $matches = [];
