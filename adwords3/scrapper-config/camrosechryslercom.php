...
$scrapper_configs["camrosechryslercom"] = array( 
	'entry_points' => array(
        'new' => 'https://www.camrosechrysler.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1',
        'used' => 'https://www.camrosechrysler.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
    ),
            ...
            foreach ($objects->pageInfo->trackingData as $obj) {
                ...
                $car_data = array(
                   ...
                   // 'trim' => $obj->trim,
                    ...
                    'url' => "https://www.camrosechrysler.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",       
                );
 ...
 add_filter("filter_camrosechryslercom_next_page", "filter_camrosechryslercom_next_page", 10, 2);       
 function filter_camrosechryslercom_next_page($next, $current_page) {
        $start_tag = 'start=';
        $end_tag = '&';
