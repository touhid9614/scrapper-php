...
$scrapper_configs['korfcontinentalsterlingcom'] = array(
    'entry_points' => array(
        'used' => 'https://www.korfcontinentalsterling.com/searchused.aspx',
        'new' => 'https://www.korfcontinentalsterling.com/searchnew.aspx'
    ),
    ...
    'next_page_regx' => '/<a href="(?<next>[^"]+)"\s*class="stat-arrow-next"[^>]+>\s*[^"]+"[^"]+"\s*aria-label="Next Page">/',
    'images_regx' => '/zoom feature element -->\s*<img src="(?<img_url>[^\?]+)/',
   // 'images_fallback_regx' => '/<meta property="og:image" content="(?<img_url>[^"]+)"/'
);

add_filter("filter_korfcontinentalsterlingcom_field_price", "filter_korfcontinentalsterlingcom_field_price", 10, 3);

function filter_korfcontinentalsterlingcom_field_price($price, $car_data, $spltd_data) {
    $prices = [];
