...
$scrapper_configs["westshoremarine"] = array(
    "entry_points" => array(
        'used' => 'https://ehwuw84xvk-1.algolianet.com/1/indexes/*/queries?x-algolia-agent=Algolia%20for%20JavaScript%20(4.9.1)%3B%20Browser%20(lite)%3B%20JS%20Helper%20(3.4.4)&x-algolia-api-key=fb58227032e79f03b9b820cbaea7f8fb&x-algolia-application-id=EHWUW84XVK',
        'new' => 'https://ehwuw84xvk-1.algolianet.com/1/indexes/*/queries?x-algolia-agent=Algolia%20for%20JavaScript%20(4.9.1)%3B%20Browser%20(lite)%3B%20JS%20Helper%20(3.4.4)&x-algolia-api-key=fb58227032e79f03b9b820cbaea7f8fb&x-algolia-application-id=EHWUW84XVK',
    ),
    ...
    'vdp_url_regex'       => '/\/inventory\/(?:new|used|certified|certified-used)-[0-9]{4}/i',
        ...
        foreach ($objects->results[0]->hits as $obj) {
            ...
            $car_data = array(
                ...
                'year' => $obj->year,
                'make' => trim($obj->make),
                'model' => trim($obj->model),
                'trim' => $obj->trim,
                'body_style' => trim($obj->body!='OTHER' ? $obj->body : ""),
            ...
            $regex       =  '/"og:title" content="[^\s*]+\s*[^\s*]+\s*[^\s*]+\s*(?<model>[^"]+)/';
            $matches = [];
            if($obj->model == "" || $obj->model == $obj->make){
                if(preg_match($regex, $response_data, $matches)) {
                    $car_data['model']=$matches['model'];
                } 
            }
...
add_filter('filter_for_fb_westshoremarine', 'filter_for_fb_westshoremarine', 10, 2);
function filter_for_fb_westshoremarine($car_data, $feed_type)
{
   
    if(strpos($car_data['all_images'],"XXXXXX6LWX1000239"))
    {
        return null;
    }
