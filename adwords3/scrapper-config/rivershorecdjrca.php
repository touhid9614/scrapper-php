...
$scrapper_configs["rivershorecdjrca"] = array( 
	'entry_points'        => array(
        'used' => 'https://www.rivershorecdjr.ca/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
        'new'  => 'https://www.rivershorecdjr.ca/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1',
    ),
    ...
    'custom_data_capture' => function ($url, $data) {
        ...
        foreach ($objects->pageInfo->trackingData as $obj) {
            ...
            $car_data = array(
                ...
                'url' 			 => "https://www.rivershorecdjr.ca{$obj->link}",
                // 'url'            => "https://www.rivershorecdjrca.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}" . ".htm", // wrong url in new cars
            );
...
add_filter("filter_rivershorecdjrca_next_page", "filter_rivershorecdjrca_next_page", 10, 2);
function filter_rivershorecdjrca_next_page($next, $current_page)
{
