<?php
    ...
    $scrapper_configs['greggyoungcdjr'] = array(
        'entry_points' => array(
            'new'   => 'https://www.greggyoungcdjrofgrandisland.com/new-vehicles/',
            'used'  => 'https://www.greggyoungcdjrofgrandisland.com/used-vehicles/'
        ),
         'use-proxy' => true,
    ...
    add_filter('filter_greggyoungcdjr_post_data', 'filter_greggyoungcdjr_post_data', 10, 3);
    add_filter('filter_greggyoungcdjr_data', 'filter_greggyoungcdjr_data');
    
    $greggyoungcdjr_nonce = '';
            
    function filter_greggyoungcdjr_post_data($post_data, $stock_type, $data)
    {
        global $greggyoungcdjr_nonce;
        if($post_data == '')
        {
            $post_data = "page=1";
        }
        ...
        if($nonce && isset($nonce)) { $greggyoungcdjr_nonce=$nonce; }
        slecho("global ajax_nonce : ".$greggyoungcdjr_nonce);
        $post_id = 4;
        ...
        return "action=im_ajax_call&perform=get_results&$post_data&_nonce=$greggyoungcdjr_nonce&_post_id=$post_id&_referer=$referer";
    }

    function filter_greggyoungcdjr_data($data)
    {
        if($data)
        {
            if(isJSON($data)){
