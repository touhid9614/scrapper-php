<?php
    ...
    $scrapper_configs['freedomfordsales'] = array(
        'entry_points' => array(
            'used'   => 'https://www.freedomfordsales.ca/used-inventory/index.htm',
            'new'    => 'https://www.freedomfordsales.ca/new-inventory/index.htm',
       ...
       'vdp_url_regex'     => '/\/(?:new|used|certified)\/[^\/]+\/[0-9]{4}-/i',
        ...
        'data_capture_regx_full' => array(        
            ...
            'fuel_type'     => '/Recommended fuel<\/span>[^>]+>[^>]+>[^>]+>(?<fuel_type>[^<]+)/',
            'description'   => '/Comments:\s*<\/strong>[^>]+>(?<description>[^<]+)/',
        ) ,
        'next_page_regx'    => '/href="(?<next>[^"]+)"\s*rel="next"/',
        'images_regx'       => '/<a href="(?<img_url>[^"]+)"\s*class="js-link">/',
        'images_fallback_regx'  => '/<meta name="og:image" content="(?<img_url>[^"]+)"/'
    );
    add_filter("filter_freedomfordsales_field_price", "filter_freedomfordsales_field_price", 10, 3);
    add_filter('filter_freedomfordsales_car_data', 'filter_freedomfordsales_car_data');
    function filter_freedomfordsales_car_data($car_data) {
    

    if($car_data['model']=='Transit-250'){
         return null;
    }
    ...
    function filter_freedomfordsales_field_price($price,$car_data, $spltd_data)
    {
        $prices = [];
