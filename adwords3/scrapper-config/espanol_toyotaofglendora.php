...
global $scrapper_configs;
 $scrapper_configs["espanol_toyotaofglendora"] = array( 
	 'entry_points' => array(
            'new'   => 'http://espanol.toyotaofglendora.com/buscar/nuevo/tp/',
            'used'  => 'http://espanol.toyotaofglendora.com/buscar/usado/tp/',
           // 'certified' => 'https://www.espanol_toyotaofglendora.com/search/certified/tp/'
        ),
        'use-proxy' => true,
        'vdp_url_regex'     => '/\/auto\/(?:nuevo|usado)-[0-9]{4}-/i',
        ...
        'data_capture_regx' => array(
            ...
            'kilometres'    => '/mileageFromOdometer"[^>]+>\s*[^"]+"[^"]+"[^"]+"(?<kilometres>[^"]+)/',
    ...
    add_filter('filter_espanol_toyotaofglendora_field_price', 'filter_espanol_toyotaofglendora_field_price',10,3);
    add_filter('filter_espanol_toyotaofglendora_field_images', 'filter_espanol_toyotaofglendora_field_images');
    
    function filter_espanol_toyotaofglendora_field_price($price,$car_data, $spltd_data)
    {
        $prices = [];
        ...
        if(preg_match($offer_regex, $spltd_data, $matches) && numarifyPrice($matches['price']) > 0) {
            $prices[] = numarifyPrice($matches['price']);
            slecho("Regex price: {$matches['price']}");
        }
    ...
    function filter_espanol_toyotaofglendora_field_images($im_urls)
    {
