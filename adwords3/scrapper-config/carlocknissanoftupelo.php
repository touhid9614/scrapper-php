...
global $scrapper_configs;
 $scrapper_configs["carlocknissanoftupelo"] = array( 
	 'entry_points' => array(
        'new' => 'https://www.carlocknissanoftupelo.com/VehicleSearchResults?search=new',
        'used' => 'https://www.carlocknissanoftupelo.com/VehicleSearchResults?search=used',
    ),
    ...
    'images_regx' => '/dealershipPhotos-[^\!]+\![^\!]+\![^\!]+\![^\!]+\![^>]+>[^>]+>[^>]+>[^>]+>[^>]+>[^>]+>[^>]+>[^"]+"[^"]+"[^"]+"[^"]+"[^"]+"[^"]+" noresponsive data-src="(?<img_url>[^"]+)/',
);

add_filter("filter_carlocknissanoftupelo_next_page", "filter_carlocknissanoftupelo_next_page", 10, 2);
add_filter("filter_carlocknissanoftupelo_field_images", "filter_carlocknissanoftupelo_field_images");


function filter_carlocknissanoftupelo_next_page($next, $current_page) {
    slecho("Filtering Next url");
    $car_type = explode('=', $current_page);
    return urlCombine($next, "?search={$car_type[count($car_type) - 1]}");
}

function filter_carlocknissanoftupelo_field_images($im_urls)
    {
        $retval = [];
