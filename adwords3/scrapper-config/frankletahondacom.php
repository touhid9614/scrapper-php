...
$scrapper_configs["frankletahondacom"] = array( 
	  'entry_points' => array(
            'new'    => 'https://www.frankletahonda.com/new-vehicles/',
            'used'   => 'https://www.frankletahonda.com/used-vehicles/'
        ),
        'use-proxy' => true,
       'refine'=>false,
    ...
    add_filter('filter_frankletahondacom_post_data', 'filter_frankletahondacom_post_data', 10, 3);
    add_filter('filter_frankletahondacom_data', 'filter_frankletahondacom_data');
    add_filter("filter_frankletahondacom_field_images", "filter_frankletahondacom_field_images");
    
    $frankletahondacom_nonce = '';
    
    function filter_frankletahondacom_post_data($post_data, $stock_type, $data)
    {
        global $frankletahondacom_nonce;
        if($post_data == '')
        {
            $post_data = "page=1";
        }
        ...
        if($nonce && isset($nonce)) { $frankletahondacom_nonce=$nonce; }
        slecho("global ajax_nonce : ".$frankletahondacom_nonce);
        $post_id = 4;
        ...
        return "action=im_ajax_call&perform=get_results&$post_data&_nonce=$frankletahondacom_nonce&_post_id=$post_id&_referer=$referer";
    }

    function filter_frankletahondacom_data($data)
    {
        if($data)
        {
            if(isJSON($data)){
    ...
    function filter_frankletahondacom_field_images($im_urls)
    {
    ...
    add_filter("filter_frankletahondacom_field_price", "filter_frankletahondacom_field_price", 10, 3);


    function filter_frankletahondacom_field_price($price,$car_data, $spltd_data)
    {
        $prices = [];
        ...
        $msrp_regex =  '/>Market Price[^>]+>\s*[^>]+>(?<price>[^<]+)/';
        $internet_regex =  '/>Leta Price[^>]+>\s*[^>]+>(?<price>[^<]+)/';
        $matches = [];
        ...
        if(preg_match($msrp_regex, $spltd_data, $matches) && numarifyPrice($matches['price']) > 0) {
           ...
           if(preg_match($internet_regex, $spltd_data, $matches) && numarifyPrice($matches['price']) > 0) {
            $prices[] = numarifyPrice($matches['price']);
            slecho("Regex Leta Price: {$matches['price']}");
        }
