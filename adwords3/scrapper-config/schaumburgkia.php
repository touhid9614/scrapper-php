...
$scrapper_configs['schaumburgkia'] = array(
    'entry_points' => array(
        'new' => 'https://www.schaumburgkia.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1',
        'used' => 'https://www.schaumburgkia.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
                ...
                $car_data = array(
                 ...
                 //  'price' => $conditional > 0 ? ($finalPrice > $conditional ? $conditional:($finalPrice==0? "please call": $finalPrice)):($finalPrice==0 ? "please call": $finalPrice),              
                   ...
                   // 'images'            => array_merge($obj->photos->user, $obj->photos->stock),    
                    // 'url' => "https://www.schaumburgkia.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
                    'url' 		 => "https://www.schaumburgkia.com{$obj->link}",            
                );
 ...
 add_filter("filter_schaumburgkia_next_page", "filter_schaumburgkia_next_page", 10, 2);       
 function filter_schaumburgkia_next_page($next, $current_page) {
