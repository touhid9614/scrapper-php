...
$scrapper_configs["mykwfordcom"] = array( 
	 'entry_points'        => array(
        'used' => 'https://www.mykwford.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
        'new'  => 'https://www.mykwford.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1',
    ),
    ...
    'custom_data_capture' => function ($url, $data) {
        ...
        foreach ($objects->pageInfo->trackingData as $obj) {
            ...
            $car_data = array(
                ...
                'url' 			 => "https://www.mykwford.com{$obj->link}",
                // 'url'            => "https://www.mykwfordcom.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}" . ".htm", // wrong url in new cars
            );
            $temp_data = HttpGet($car_data['url']);

            $description_regex    = '/<meta name="og:description" content="(?<description>[^"]+)/';
...
add_filter("filter_mykwfordcom_next_page", "filter_mykwfordcom_next_page", 10, 2);
function filter_mykwfordcom_next_page($next, $current_page)
{
