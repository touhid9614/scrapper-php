...
$scrapper_configs["chapfordsalescom"] = array( 
	 'entry_points' => array(
        'new' => 'https://www.chapfordsales.com/searchnew.aspx',
        'used' => 'https://www.chapfordsales.com/searchused.aspx'
    ),
    ...
    'data_capture_regx' => array(
         'vin'           => '/VIN #:\s*[^>]+>[^>]+>(?<vin>[^<]+)/',
        'url' => '/href="(?<url>[^"]+)">\s*<span[^>]+>(?<title>(?<year>[0-9]{4})\s*(?<make>[^\s]+)\s*(?<model>[^<]+))/',
        //'title' => '/href="(?<url>[^"]+)">\s*<span[^>]+>(?<title>(?<year>[0-9]{4})\s*(?<make>[^\s]+)\s*(?<model>[^\s]+))/',
        'year' => '/href="(?<url>[^"]+)">\s*<span[^>]+>(?<title>(?<year>[0-9]{4})\s*(?<make>[^\s]+)\s*(?<model>[^<]+))/',
        'make' => '/href="(?<url>[^"]+)">\s*<span[^>]+>(?<title>(?<year>[0-9]{4})\s*(?<make>[^\s]+)\s*(?<model>[^<]+))/',
        'model' => '/href="(?<url>[^"]+)">\s*<span[^>]+>(?<title>(?<year>[0-9]{4})\s*(?<make>[^\s]+)\s*(?<model>[^<]+))/',
        ...
        'price' => '/Internet Price\s*<\/span><span[^>]+>\$(?<price>[^<]+)/',
    ),
...
add_filter("filter_chapfordsalescom_field_price", "filter_chapfordsalescom_field_price", 10, 3);

function filter_chapfordsalescom_field_price($price, $car_data, $spltd_data) {
    $prices = [];
    ...
    $msrp_regex = '/Chaparral Price:\s*<\/span><span[^>]+>\$(?<price>[^<]+)/';
