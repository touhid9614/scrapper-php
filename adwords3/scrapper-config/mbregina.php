...
$scrapper_configs['mbregina'] = array(
    'entry_points' => array(
    ...
    'custom_data_capture' => function ($url, $data) {
        ...
        foreach ($objects->results[0]->hits as $obj) {
            $car_data = array(
                ...
                'stock_type'     => $obj->type=='Certified Pre-Owned'? 'cpo' : ($obj->type=='Certified'? 'cpo' :($obj->type=='Pre-Owned' ? 'used' : 'new')),
               // 'stock_type'        => $obj->type=='Certified Pre-Owned'? 'certified': strtolower($obj->type),
...
function filter_mbregina_post_data($post_data, $stock_type)
{
 ...
 add_filter("filter_mbregina_field_images", "filter_mbregina_field_images", 10, 2);


function filter_mbregina_field_images($im_urls, $car_data)
{
      if(count($im_urls)<2)
            {
            return [];
...
add_filter('filter_mbregina_car_data', 'filter_mbregina_car_data');

function filter_mbregina_car_data($car_data) {

    if ($car_data['stock_number'] === 'T0633' | $car_data['stock_number'] === '011088' | $car_data['stock_number'] === '200040') {
