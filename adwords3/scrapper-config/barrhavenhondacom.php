...
global $scrapper_configs;
 $scrapper_configs["barrhavenhondacom"] = array( 
	 'entry_points' => array(
             'used'  => 'https://www.barrhavenhonda.com/used/',
            'new'   => 'https://www.barrhavenhonda.com/new/',
        ...
        'data_capture_regx' => array(
            ...
            'price'               => '/Price:<\/span>\s*<span[^>]+><meta[^>]+>(?:[^>]+>|)<span[^>]+>(?<price>\$[0-9,]+)/',
            'kilometres'          => '/STK#\s*(?<stock_number>[^\/]+)\/\s*(?<kilometres>[^\s]+)\s*[^\/]+\/\s*(?<engine>[^<]+)/',
            'stock_number'        => '/STK#\s*(?<stock_number>[^\/]+)/',
            'engine'              => '/STK#\s*(?<stock_number>[^\/]+)\/\s*(?<kilometres>[^\s]+)\s*[^\/]+\/\s*(?<engine>[^<]+)/',
        ...
        'description'             => '/itemprop="description"[^>]+>(?<description>[^<]+)/',
            ...
            'drivetrain'          => '/itemprop="vehicleDrivetrain">\s*(?<drivetrain>[^<]+)/',
        ) ,
        'next_page_regx'    => '/class="active"><a\s*href="">[^<]+<\/a><\/li>\s*<li><a\s*href="(?<next>[^"]+)/',
        'images_regx'       => '/imgError\(this\)\;"\s*(?:data-src|src)="(?<img_url>[^"]+)/'
    );
    
    add_filter("filter_barrhavenhondacom_field_price", "filter_barrhavenhondacom_field_price", 10, 3);
    add_filter("filter_barrhavenhondacom_field_images", "filter_barrhavenhondacom_field_images");
    function filter_barrhavenhondacom_field_price($price,$car_data, $spltd_data)
    {
        $prices = [];
        ...
        if($price && numarifyPrice($price) > 0) {
            ...
            slecho("barrhavenhondacom Price: $price");
        }
    ...
    function filter_barrhavenhondacom_field_images($im_urls)
    {
