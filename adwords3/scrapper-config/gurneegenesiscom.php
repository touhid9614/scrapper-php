...
$scrapper_configs["gurneegenesiscom"] = array( 
	'entry_points' => array(
            'used' => 'https://www.gurneegenesis.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
        'new' => 'https://www.gurneegenesis.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1'
    ),
        ...
        'custom_data_capture' => function($url, $data) {
            ...
            foreach ($objects->pageInfo->trackingData as $obj) {
                ...
                $car_data = array(
                   ...
                   // 'url' => "https://www.gurneegenesis.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
                    'url' => "https://www.gurneegenesis.com{$obj->link}",          
                );
                    
                    $imgs = [];
                    foreach ($obj->images as $key => $value) {
                        $imgs[] = $value->uri . "?impolicy=resize&w=768";
 ...
 add_filter("filter_gurneegenesiscom_next_page", "filter_gurneegenesiscom_next_page", 10, 2);       
 function filter_gurneegenesiscom_next_page($next, $current_page) {
