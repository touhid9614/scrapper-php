<?php
    ...
    $scrapper_configs['yorkdaleford'] = array(
        'entry_points' => array(
            'all'   => 'http://yorkdaleford.com/data/inventory.json?ts=' . time(),
        ),
        'use-proxy' => true,
        'vdp_url_regex'     => '/\/view\/(?:new|used)-[0-9]{4}-/i',
        'ajax_url_match'    => '/api/v1/lead',
        'ajax_resp_match'   => '"Type":"RequestMoreInformation"',
        'picture_selectors' => ['.lb-image'],
        ...
        'custom_data_capture' => function($url, $data){
            ...
            foreach($objects->vehicles as $obj)
            {
                ...
                $car_data = array(
                    ...
                    'url'               => "http://yorkdaleford.com/view/$slug/",
                    'exterior_color'    => $obj->exteriorcolor
                );
                
                $details_url = "http://yorkdaleford.leadbox.info/data/{$obj->id}.json?{$obj->timestamp}";
