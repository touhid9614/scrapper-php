...
$scrapper_configs["lucasmotorcompanycom"] = array( 
	'entry_points' => array(
        'used' => 'https://www.lucasmotorcompany.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1&accountId=lucasmotorfordfd',
        'new' => 'https://www.lucasmotorcompany.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1&accountId=lucasmotorfordfd'
    ),
              ...
              $conditional=(int)str_replace([",","\$"], ["",""],trim(($obj->pricing->SICFRule)));
                $car_data = array(
                   ...
                   // 'images'            => array_merge($obj->photos->user, $obj->photos->stock),    
                    'url' => "https://www.lucasmotorcompany.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
 ...
 add_filter("filter_lucasmotorcompanycom_next_page", "filter_lucasmotorcompanycom_next_page", 10, 2);       
 function filter_lucasmotorcompanycom_next_page($next, $current_page) {
