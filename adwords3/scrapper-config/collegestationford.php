<?php
    ...
    $scrapper_configs['collegestationford'] = array(
        'entry_points' => array(
            'new'    => 'https://www.collegestationford.com/new-vehicles/',
            'used'   => 'https://www.collegestationford.com/used-vehicles/'
        ),
        //'use-proxy' => true,
        'proxy-area'        => 'FL',
    ...
    add_filter('filter_collegestationford_post_data', 'filter_collegestationford_post_data',10, 3);
    add_filter('filter_collegestationford_data', 'filter_collegestationford_data');
    add_filter("filter_collegestationford_field_images", "filter_collegestationford_field_images");
    
    $collegestationford_nonce = '';
    
    function filter_collegestationford_post_data($post_data, $stock_type, $data)
    {
        global $collegestationford_nonce;
        if($post_data == '')
        {
            $post_data = "page=1";
        }
        ...
        if($nonce && isset($nonce)) { $collegestationford_nonce=$nonce; }
        slecho("global ajax_nonce : ".$collegestationford_nonce);
        $post_id = 4;
        ...
        return "action=im_ajax_call&perform=get_results&$post_data&show_all_filters=false&_nonce=$collegestationford_nonce&_post_id=$post_id&_referer=$referer";
    }

    function filter_collegestationford_data($data)
    {
        if($data)
        {
            if(isJSON($data)){
    ...
    function filter_collegestationford_field_images($im_urls)
    {
