<?php
    ...
    $scrapper_configs['portlavacadodge'] = array(
        'entry_points' => array(
            'new'    => 'https://www.portlavacadodge.com/new-vehicles/',
            'used'   => 'https://www.portlavacadodge.com/used-vehicles/'
        ),
        'use-proxy' => true,
    ...
    add_filter('filter_portlavacadodge_post_data', 'filter_portlavacadodge_post_data', 10, 3);
    add_filter('filter_portlavacadodge_data', 'filter_portlavacadodge_data');

    $portlavacadodge_nonce = '';
    
    function filter_portlavacadodge_post_data($post_data, $stock_type, $data)
    {
        global $portlavacadodge_nonce;
        if($post_data == '')
        {
            $post_data = "page=1";
        }
        ...
        if($nonce && isset($nonce)) { $portlavacadodge_nonce=$nonce; }
        slecho("global ajax_nonce : ".$portlavacadodge_nonce);
        $post_id = 4;
        ...
        return "action=im_ajax_call&perform=get_results&_post_id=$post_id&$post_data&show_all_filters=false&_nonce=$portlavacadodge_nonce&_referer=$referer";
    }

    function filter_portlavacadodge_data($data)
    {
        if($data)
        {
            if(isJSON($data)){
