<?php
    ...
    $scrapper_configs['maseratiaustin'] = array(
        'entry_points' => array(
            'new'    => 'https://www.maseratiaustin.com/new-vehicles/',
            'used'   => 'https://www.maseratiaustin.com/used-vehicles/'
        ),
        'use-proxy' => true,
    ...
    add_filter('filter_maseratiaustin_post_data', 'filter_maseratiaustin_post_data', 10, 3);
    add_filter('filter_maseratiaustin_data', 'filter_maseratiaustin_data');
    add_filter("filter_maseratiaustin_field_images", "filter_maseratiaustin_field_images");
    
    $maseratiaustin_nonce = '';
    
    function filter_maseratiaustin_post_data($post_data, $stock_type, $data)
    {
        global $maseratiaustin_nonce;
        if($post_data == '')
        {
            $post_data = "page=1";
        }
        ...
        if($nonce && isset($nonce)) { $maseratiaustin_nonce=$nonce; }
        slecho("global ajax_nonce : ".$maseratiaustin_nonce);
        $post_id = 6;
        ...
        return "action=im_ajax_call&perform=get_results&$post_data&_nonce=$maseratiaustin_nonce&_post_id=$post_id&_referer=$referer";
    }

    function filter_maseratiaustin_data($data)
    {
        if($data)
        {
            if(isJSON($data)){
    ...
    function filter_maseratiaustin_field_images($im_urls)
    {
