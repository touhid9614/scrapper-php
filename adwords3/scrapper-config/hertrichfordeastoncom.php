...
$scrapper_configs["hertrichfordeastoncom"] = array( 
	"entry_points" => array(
	    'new' => 'https://www.hertrichfordeaston.com/new-inventory/index.htm',
        'used' => 'https://www.hertrichfordeaston.com/used-inventory/index.htm'
    ),
...
add_filter("filter_hertrichfordeastoncom_field_price", "filter_hertrichfordeastoncom_field_price", 10, 3);

function filter_hertrichfordeastoncom_field_price($price, $car_data, $spltd_data) {
    $prices = [];
    ...
    $wholesale_regex = '/Final Price[^>]+>[^>]+>[^>]+>[^>]+>[^>]+>[^>]+>(?<price>[^<]+)/';
    $internet_regex = '/Conditional Final Price[^>]+>[^>]+>[^>]+>[^>]+>(?<price>[^<]+)/';
