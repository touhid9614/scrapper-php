...
$scrapper_configs["whitesenergymotorsnet"] = array( 
	 'entry_points' => array(
        'used' => 'https://www.whitesenergymotors.net/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
        'new' => 'https://www.whitesenergymotors.net/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1'
    ),
                ...
                $car_data = array(
                  ...
                  //  'options'           => isset($obj->installed_options)?$obj->installed_options:array(),
                   // 'images'            => array_merge($obj->photos->user, $obj->photos->stock),    
                    'url' => "https://www.whitesenergymotors.net/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
 ...
 add_filter("filter_whitesenergymotorsnet_next_page", "filter_whitesenergymotorsnet_next_page", 10, 2);       
 function filter_whitesenergymotorsnet_next_page($next, $current_page) {
