...
global $scrapper_configs;
 $scrapper_configs["sill_terharmotorscom"] = array( 
	 'entry_points' => array(
           'used' => 'https://www.sill-terharmotors.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
        'new' => 'https://www.sill-terharmotors.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1'
    ),
            ...
            foreach ($objects->pageInfo->trackingData as $obj) {
                ...
                $car_data = array(
                   ...
                   // 'images'            => array_merge($obj->photos->user, $obj->photos->stock),    
                    'url' => "https://www.sill-terharmotors.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
 ...
 add_filter("filter_sill_terharmotorscom_next_page", "filter_sill_terharmotorscom_next_page", 10, 2);       
 function filter_sill_terharmotorscom_next_page($next, $current_page) {
