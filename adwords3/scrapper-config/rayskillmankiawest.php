...
global $scrapper_configs;
 $scrapper_configs["rayskillmankiawest"] = array( 
	 'entry_points' => array(
            'new'   => 'https://www.rayskillmankiawest.com/new-vehicles/',
            'used'  => 'https://www.rayskillmankiawest.com/used-vehicles/'
        ),
         'use-proxy' => true,
        ...
        'init_method'       => 'GET',
        'next_method'       => 'POST',
        'picture_selectors' => ['.owl-item div img'],
        'picture_nexts'     => ['.owl-next'],
        'picture_prevs'     => ['.owl-prev'],
        'details_start_tag' => '<div class="grid-view-results-wrapper">',
        //'details_end_tag'   => '<div class="resultsPagination',
...
add_filter('filter_rayskillmankiawest_post_data', 'filter_rayskillmankiawest_post_data', 10, 3);
add_filter('filter_rayskillmankiawest_data', 'filter_rayskillmankiawest_data');
add_filter("filter_rayskillmankiawest_field_images", "filter_rayskillmankiawest_field_images");

$rayskillmankiawest_nonce = '';

function filter_rayskillmankiawest_post_data($post_data, $stock_type, $data) {
    global $rayskillmankiawest_nonce;
    if ($post_data == '') {
        $post_data = "page=1";
    }
    ...
    if ($nonce && isset($nonce)) {
        $rayskillmankiawest_nonce = $nonce;
    }
    slecho("global ajax_nonce : " . $rayskillmankiawest_nonce);
    $post_id = 4;
    ...
    return "action=im_ajax_call&perform=get_results&$post_data&_nonce=$rayskillmankiawest_nonce&_post_id=$post_id&_referer=$referer";
}

function filter_rayskillmankiawest_data($data) {
    if ($data) {
        if (isJSON($data)) {
...
function filter_rayskillmankiawest_field_images($im_urls) {
...
add_filter("filter_rayskillmankiawest_next_page", "filter_rayskillmankiawest_next_page", 10, 2);

function filter_rayskillmankiawest_next_page($next_page_regex) {
