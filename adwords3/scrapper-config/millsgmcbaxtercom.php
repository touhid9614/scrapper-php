...
$scrapper_configs["millsgmcbaxtercom"] = array( 
	'entry_points' => array(
        'used' => 'https://www.millsgmcbaxter.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
        'new' => 'https://www.millsgmcbaxter.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1'
    ),
            ...
            foreach ($objects->pageInfo->trackingData as $obj) {
                ...
                //    'price' => $obj->pricing->finalPrice,
                   ...
                   // 'images'            => array_merge($obj->photos->user, $obj->photos->stock),    
                    'url' => "https://www.millsgmcbaxter.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
 ...
 add_filter("filter_millsgmcbaxtercom_next_page", "filter_millsgmcbaxtercom_next_page", 10, 2);       
 function filter_millsgmcbaxtercom_next_page($next, $current_page) {
...
add_filter('filter_millsgmcbaxtercom_field_images','filter_millsgmcbaxtercom_field_images');
function filter_millsgmcbaxtercom_field_images($im_urls)
 {
    unset($im_urls[0]);
    return $im_urls;
}