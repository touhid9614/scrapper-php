<?php
    ...
    $scrapper_configs['fairbankschevy'] = array(
        'entry_points' => array(
            'new'    => 'https://www.fairbankschevy.com/new-vehicles/',
            'used'   => 'https://www.fairbankschevy.com/used-vehicles/'
        ),
        'use-proxy' => true,
    ...
    add_filter('filter_fairbankschevy_post_data', 'filter_fairbankschevy_post_data', 10, 3);
    add_filter('filter_fairbankschevy_data', 'filter_fairbankschevy_data');
    add_filter("filter_fairbankschevy_field_price", "filter_fairbankschevy_field_price", 10, 3);
    add_filter("filter_fairbankschevy_field_images", "filter_fairbankschevy_field_images");
    
    $fairbankschevy_nonce = '';
            
    function filter_fairbankschevy_post_data($post_data, $stock_type, $data)
    {
        global $fairbankschevy_nonce;
        if($post_data == '')
        {
            $post_data = "page=1";
        }
        ...
        if($nonce && isset($nonce)) { $fairbankschevy_nonce=$nonce; }
        slecho("global ajax_nonce : ".$fairbankschevy_nonce);
        $post_id = 4;
        ...
        return "action=im_ajax_call&perform=get_results&$post_data&_nonce=$fairbankschevy_nonce&_post_id=$post_id&_referer=$referer";
    }


   function filter_fairbankschevy_data($data)
    {
        if($data)
        {
            if(isJSON($data)){
    ...
    function filter_fairbankschevy_field_price($price,$car_data, $spltd_data)
    {
        $prices = [];
        ...
        $selling_regex =  '/Selling Price<\/span>\s*<span[^>]+>(?<price>\$[0-9,]+)/';
    ...
    function filter_fairbankschevy_field_images($im_urls)
    {
