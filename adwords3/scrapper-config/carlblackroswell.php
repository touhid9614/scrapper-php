<?php
    ...
    $scrapper_configs['carlblackroswell'] = array(
        'entry_points' => array(
            'new'  => 'http://www.carlblackroswell.com/new-inventory/index.htm',
            'used' => 'http://www.carlblackroswell.com/used-inventory/index.htm'
        ),
        ...
        'data_capture_regx' => array(
            ...
            'price'         => '/class="price-block our-price withsavings"\s*>[\s\S]*?<span class="price">\s*(?<price>\$[0-9,]+)/',
            ...
            'msrp'          => '/MSRP\s*<\/span>\s*<span class="price">\s*(?<msrp>\$[0-9,]+)/',
        ),
        'data_capture_regx_full' => array(
           ...
           // 'original'      => '/<span class="ctabox-price-label">\s*MSRP\s*<\/span>\s*<span class="ctabox-price">\s*(?<original>[\$0-9,]+)/',
            'price'         => '/ctabox-price-label">\s*Carl\sBlack\sPrice\s*<\/span>\s*<span class="ctabox-price">\s*(?<price>[\$0-9,]+)/'
        ),
        'next_query_regx'   => '/data-(?<param>page)="(?<value>[0-9]*)" class="next">/',
        'images_regx'       => '/<meta itemprop="image" content="(?<img_url>[^"]+)">/'
    );
    
    add_filter('filter_carlblackroswell_post_data', 'filter_carlblackroswell_post_data', 10, 3);
    add_filter('filter_carlblackroswell_data', 'filter_carlblackroswell_data');
    add_filter("filter_carlblackroswell_field_price", "filter_carlblackroswell_field_price", 10, 3);
    add_filter('filter_carlblackroswell_field_title', 'filter_carlblackroswell_field_title');
    add_filter("filter_carlblackroswell_field_images", "filter_carlblackroswell_field_images");

    $carlblackroswell_nonce = '';
    
    function filter_carlblackroswell_post_data($post_data, $stock_type, $data)
    {
        global $carlblackroswell_nonce;
        if($post_data == '')
        {
            $post_data = "page=1";
        }
        ...
        if($nonce && isset($nonce)) { $carlblackroswell_nonce=$nonce; }
        slecho("global ajax_nonce : ".$carlblackroswell_nonce);
        $post_id = 5;
        ...
        return "action=im_ajax_call&perform=get_results&$post_data&_nonce=$carlblackroswell_nonce&_post_id=$post_id&_referer=$referer";
    }

    function filter_carlblackroswell_data($data)
    {
        if($data)
        {
            if(isJSON($data)){
    ...
    function filter_carlblackroswell_field_price($price,$car_data, $spltd_data)
    {
        $prices = [];
    ...
    function filter_carlblackroswell_field_images($im_urls)
    {
        if(count($im_urls) < 2) { return array(); }
        return $im_urls;
    }
    
     function filter_carlblackroswell_field_title($title)
    {
        return trim($title);
    }
