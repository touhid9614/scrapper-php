...
$scrapper_configs["albertacreditcentrecom"] = array( 
	"entry_points" => array(
...
'vdp_url_regex'         => '/\/inventory\/(?:used|certified-used)-[0-9]{4}-/',
        'use-proxy'         => true,
        'refine'            => false,
        'init_method'       => 'POST',
        'next_method'       => 'POST',
        'picture_selectors' => ['.vehicle-image-bg.swiper-lazy-loaded'],
 ...
 foreach($objects->results[0]->hits as $obj)
        {
            ...
            $car_data = array(
                ...
                'stock_type'        => ($obj->type =='Used')?'used':'certified',
                'year'              => $obj->year,
                'make'              => $obj->make,
                'model'             => $obj->model,
                'trim'              => $obj->trim,
...
function filter_albertacreditcentrecom_field_images($im_urls) {
    if (count($im_urls) < 3) {
        return array();
    }
    return $im_urls;
