...
$scrapper_configs['mikeburkartford'] = array(
    'entry_points' => array(
         'new' => 'https://www.mikeburkartford.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1',
         'used' => 'https://www.mikeburkartford.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
            ...
            foreach ($objects->pageInfo->trackingData as $obj) {
                ...
                $car_data = array(
                   ...
                   // 'images'            => array_merge($obj->photos->user, $obj->photos->stock),    
                    'url' => "https://www.mikeburkartford.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
                                
                );

                 $response_data = HttpGet($car_data['url']);
            $regex = '/Dealer Notes<\/h3>[^>]+>(?:<div>|[^>]+>)(?<description>[\s\S]*?(?=<\/div>))/';
            $matches = [];
 ...
 add_filter("filter_mikeburkartford_next_page", "filter_mikeburkartford_next_page", 10, 2);       
 function filter_mikeburkartford_next_page($next, $current_page) {
    ...
    add_filter("filter_mikeburkartford_field_description", "filter_mikeburkartford_field_description");

    
    function filter_mikeburkartford_field_description($description)
    {
