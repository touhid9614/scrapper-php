<?php
    ...
    $scrapper_configs['westridgegmc'] = array(
        'entry_points' => array(
            'new'   => 'http://www.westridgegmc.com/new-inventory/index.htm',
            'used'  => 'http://www.westridgegmc.com/used-inventory/index.htm'
        ),
        'use-proxy' => true,
        'new'   => array(
        ...
        'used'   => array(
            ...
            'details_end_tag'   => '<div class="ft">',
            'details_spliter'   => '<div class="item-compare">',
            'must_contain_regx' => '/Stock #:[^>]+>[^>]+>(?<stock_number>3-[0-9]*[aA])/',
            'data_capture_regx' => array(
                'stock_number'  => '/Stock #:[^>]+>[^>]+>(?<stock_number>[^<]+)/',
                'title'         => '/class="url" *href="(?<url>[^"]+)"> *(?<title>(?<year>[^ ]+) *(?<make>[^ ]+) *(?<model>[^ <]+)[^<]*)/',
                'year'          => '/class="url" *href="(?<url>[^"]+)"> *(?<title>(?<year>[^ ]+) *(?<make>[^ ]+) *(?<model>[^ <]+)[^<]*)/',
