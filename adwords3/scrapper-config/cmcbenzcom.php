...
$scrapper_configs["cmcbenzcom"] = array( 
	"entry_points" => [
		'new' => 'https://www.cmcbenz.com/new-vehicles/',
        'used' => 'https://www.cmcbenz.com/used-vehicles/'
    ],
    'use-proxy' => true,
    ...
    'data_capture_regx' => array(
        ...
        'price' => '/(?:MSRP|Price)<\/span>\s*<span class="price">(?<price>\$[0-9,]+)/',
       
    ),
    'data_capture_regx_full' => array(
        'stock_number' => '/<span class="vinstock-label">Stock[^>]+>[^>]+>(?<stock_number>[^<]+)/',
...
add_filter('filter_cmcbenzcom_post_data', 'filter_cmcbenzcom_post_data', 10, 3);
add_filter('filter_cmcbenzcom_data', 'filter_cmcbenzcom_data');

$cmcbenzcom_nonce = '';

function filter_cmcbenzcom_post_data($post_data, $stock_type, $data) {
    global $cmcbenzcom_nonce;
    if ($post_data == '') {
        $post_data = "page=1";
    }
    ...
    if ($nonce && isset($nonce)) {
        $cmcbenzcom_nonce = $nonce;
    }
    slecho("global ajax_nonce : " . $cmcbenzcom_nonce);
    $post_id = 4;
    ...
    if ($stock_type == 'used') {
     ...
     return "action=im_ajax_call&perform=get_results&$post_data&_nonce=$cmcbenzcom_nonce&_post_id=$post_id&_referer=$referer";           
}

function filter_cmcbenzcom_data($data) {
    if ($data) {
        if (isJSON($data)) {
...
add_filter("filter_cmcbenzcom_field_price", "filter_cmcbenzcom_field_price", 10, 3);

function filter_cmcbenzcom_field_price($price, $car_data, $spltd_data) {
    $prices = [];
    ...
    $msrp_regex = '/MSRP<\/strong>[^>]+>\s*[^>]+>\s*[^>]+>\s*[^>]+>\s*(?<price>[^<]+)/';
