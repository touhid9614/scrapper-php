...
$scrapper_configs["oregansgreenlightsouthshorecom"] = array(
    'entry_points' => array(
        'used' => 'https://www.oregansgreenlightsouthshore.com/used-cars/?search.location.id=32&search.start-index=0&do-search=1&advanced=&system.view-type.code=ajax',
    ),
        ...
        'custom_data_capture' => function($url, $data) {
            ...
            foreach ($objects->oreg->wuvs->results->vehicles as $obj) {
                ...
                $car_data = array(
                    ...
                    'url' => "https://www.oregansgreenlightsouthshore.com/used-cars/{$obj->year}-{$obj->vehicleMake->name}-{$obj->vehicleMakeModel->name}-"
...
add_filter('filter_oregansgreenlightsouthshorecom_car_data', 'filter_oregansgreenlightsouthshorecom_car_data');
add_filter("filter_oregansgreenlightsouthshorecom_next_page", "filter_oregansgreenlightsouthshorecom_next_page", 10, 2);

function filter_oregansgreenlightsouthshorecom_car_data($car_data) {
    ...
    if ($car_data['stock_type'] == 'new') {
        ...
        $car_data['url'] = str_replace('https://', 'https://www.oregansgreenlightsouthshore.com', $car_data['url']);
    ...
    if (isset($car_data['vin']) && $car_data['vin']) {
        ...
        $in_cookies = '';
        $out_cookies = '';
        $response_data = HttpPost($api_url, $post_data, $in_cookies, $out_cookies, false, true, 'application/x-www-form-urlencoded', ['origin' => 'https://www.oregansgreenlightsouthshore.com']);
...
function filter_oregansgreenlightsouthshorecom_next_page($next, $current_page) {
