...
$scrapper_configs["macdonaldcadillacca"] = array( 
	 'entry_points' => array(
    ...
    foreach($objects->results[0]->hits as $obj)
        {
            ...
            if($car_data['stock_number']=='22856N')
            {
                continue;
            }
...
add_filter('filter_for_fb_macdonaldcadillacca', 'filter_for_fb_macdonaldcadillacca', 10, 2);

function filter_for_fb_macdonaldcadillacca($car_data, $feed_type)
{
    

    if (in_array($feed_type, ['google-merchant', 'google-marchant', 'google_merchant', 'google_marchant'])) {
        
        $car_data['body_style']=strtolower($car_data['body_style']);
        $pos=strripos($car_data['body_style'],'s');
        $car_data['body_style']=substr($car_data['body_style'],0,$pos);
