<?php
    ...
    $scrapper_configs['hondasbynelson'] = array(
        'entry_points' => array(
    ...
    'vdp_url_regex'            => '/\/inventory\/(?:new|used|certified-used)-[0-9]{4}-/',
     'srp_page_regex'          => '/\/(?:new|used|certified-used)-vehicles/i',
    'use-proxy' => true,
    'refine' => false,
    'init_method' => 'POST',
    'next_method' => 'POST',
    ...
    'custom_data_capture' => function($url, $data) {
        ...
        foreach ($objects->results[0]->hits as $obj) {
          ...
          //  $images_regex = '/<img class="lazyOwl" (?:data-src|src)="(?<img_url>[^"]+)"/';
          $images_regex = '/class="info-left">\s*<img src="(?<img_url>[^"]+)"\s*alt/';
          $matches = [];
...
add_filter('filter_for_fb_hondasbynelson', 'filter_for_fb_hondasbynelson', 10, 2);

function filter_for_fb_hondasbynelson($car_data, $feed_type)
{
   if(strpos($car_data['all_images'],"Coming-Soon") ){
               return null;
    }
