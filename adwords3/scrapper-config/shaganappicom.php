...
$scrapper_configs["shaganappicom"] = array( 
'entry_points' => array(
             'used' => 'https://www.shaganappi.com/used-inventory/index.htm',
             'new'  => 'https://www.shaganappi.com/new-inventory/index.htm',
       ...
       'data_capture_regx' => array(
          ...
          'transmission' => '/>Transmission[^>]+>\s*[^>]+>(?<transmission>[^\<]+)/',
            'exterior_color' => '/<dt>Exterior Colour[^>]+>\s*[^>]+>(?<exterior_color>[^<]+)/',
           'interior_colour' => '/<dt>Interior Colour[^>]+>\s*[^>]+>(?<interior_color>[^<]+)/',
    ),
    ...
    add_filter("filter_shaganappicom_field_price", "filter_shaganappicom_field_price", 10, 3);
    add_filter("filter_shaganappicom_field_images", "filter_shaganappicom_field_images");
    
    
    function filter_shaganappicom_field_price($price,$car_data, $spltd_data)
    {
        $prices = [];
        ...
        $final_regex   =  '/Sale Price[^>]+>[^>]+>[^>]+>[^>]+>(?<price>[^<]+)/';
        $cond_final_regex =  '/Final Price[^>]+>[^>]+>[^>]+>[^>]+>(?<price>[^<]+)/';
        $retail_regex     =  '/>Price<span[^>]+>[^>]+>[^>]+>[^>]+>(?<price>\$[0-9,]+)/';
         $internet_regex     =  '/Shaganappi Price<span[^>]+>[^>]+>[^>]+>[^>]+>(?<price>\$[0-9,]+)/';
    ...
    function filter_shaganappicom_field_images($im_urls)
    {
