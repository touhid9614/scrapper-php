...
$scrapper_configs["olgomotorsconz"] = array( 
		 'entry_points' => array(
        'used' => 'https://www.olgomotors.co.nz/vehicles',
   ...
   'picture_selectors' => ['.slick-lightbox-slick-img'],
    ...
    'data_capture_regx' => array(
       ...
       'stock_number' => '/vehicle-specs specs-list">\s*Stock#\s*(?<stock_number>[^\s*]+)/',
    ),
    ...
    'next_page_regx' => '/<a class="btn-next" href="(?<next>[^"]+)">/',
    'images_regx' => '/<img src="(?<img_url>[^"]+)"[^>]+><\/a><\/li>/'
);

add_filter("filter_olgomotorsconz_field_images", "filter_olgomotorsconz_field_images");
function filter_olgomotorsconz_field_images($im_urls)
{
    $new_im_urls = [];
    $url_base = "https://www.olgomotors.co.nz";
     foreach ($im_urls as $im_url)
    {
        $new_im_url = preg_replace("/https:\/\/www.olgomotors.co.nz\/vehicle\/[^\/]+/", $url_base, $im_url);
