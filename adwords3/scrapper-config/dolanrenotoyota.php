...
$scrapper_configs['dolanrenotoyota'] = array(
    'entry_points' => array(
        
        'used' => 'https://www.dolanrenotoyota.com/used-inventory/index.htm',
        'new' => 'https://www.dolanrenotoyota.com/new-inventory/index.htm',
    ),
    'vdp_url_regex' => '/\/(?:new|used|certified)\/[^\/]+\/[0-9]{4}-/i',
    'use-proxy' => true,
    'picture_selectors' => ['li.slider-slide img'],
    ...
    'data_capture_regx_full' => array(
        ...
        'trim' => '@"trim": "(?<trim>[^"]+)@',
        'price' => '/Dolan Price<\/span><\/dt><dd [^>]+>(?<price>\$[0-9,]+)/',
    ),
    'next_page_regx' => '/href="(?<next>[^"]+)"\s*rel="next"/',
     'images_regx' => '/id[^"]+"[^"]+"src[^"]+"[^"]+"(?<img_url>[^"]+)[^"]+"[^"]+"thumbnail/',
    'images_fallback_regx' => '/<meta name="og:image" content="(?<img_url>[^"]+)"/'
);
add_filter("filter_dolanrenotoyota_field_price", "filter_dolanrenotoyota_field_price", 10, 3);
add_filter("filter_dolanrenotoyota_field_images", "filter_dolanrenotoyota_field_images");

function filter_dolanrenotoyota_field_price($price, $car_data, $spltd_data) {
    $prices = [];
...
function filter_dolanrenotoyota_field_images($im_urls) {
