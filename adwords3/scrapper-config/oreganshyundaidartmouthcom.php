...
$scrapper_configs["oreganshyundaidartmouthcom"] = array(
    'entry_points' => array(
        'new' => 'https://oserv2.oreganscdn.com/api/vehicle-inventory-search/?search.vehicle-make-ids[]=19&search.vehicle-inventory-type-ids=1&do-search=1&search.results-offset=0&search.results-limit=50&app.referrer=https://www.oreganshyundaidartmouth.com',
        'used' => 'https://www.oreganshyundaidartmouth.com/used-cars/?search.location.id=13&search.start-index=0&do-search=1&advanced=&system.view-type.code=ajax',
    ),
        ...
        'custom_data_capture' => function($url, $data) {
            ...
            foreach ($objects->oreg->wuvs->results->vehicles as $obj) {
                ...
                $car_data = array(
                    ...
                    'url' => "https://oreganshyundaidartmouth.com/used-cars/{$obj->year}-{$obj->vehicleMake->name}-{$obj->vehicleMakeModel->name}-"
...
add_filter('filter_oreganshyundaidartmouthcom_car_data', 'filter_oreganshyundaidartmouthcom_car_data');
add_filter("filter_oreganshyundaidartmouthcom_next_page", "filter_oreganshyundaidartmouthcom_next_page", 10, 2);

function filter_oreganshyundaidartmouthcom_car_data($car_data) {

    if ($car_data['make'] != 'Hyundai') {
        return [];
    }
    ...
    if ($car_data['stock_type'] == 'new') {
        ...
        $car_data['url'] = str_replace('https://', 'https://oreganshyundaidartmouth.com', $car_data['url']);
    ...
    if (isset($car_data['vin']) && $car_data['vin']) {
        ...
        $in_cookies = '';
        $out_cookies = '';
        $response_data = HttpPost($api_url, $post_data, $in_cookies, $out_cookies, false, true, 'application/x-www-form-urlencoded', ['origin' => 'https://oreganshyundaidartmouth.com']);
...
function filter_oreganshyundaidartmouthcom_next_page($next, $current_page) {
