...
$scrapper_configs["oregansnissandartmouth"] = array(
   'entry_points' => array(
          'used'  => 'https://www.oregansnissandartmouth.com/used-cars/?search.location.id=17&search.start-index=0&do-search=1&advanced=&system.view-type.code=ajax',
          'new'   => 'https://oserv2.oreganscdn.com/api/vehicle-inventory-search/?search.vehicle-make-ids[]=35&search.vehicle-inventory-type-ids=1&do-search=1&search.results-offset=0&search.results-limit=50&app.referrer=https://www.oregansnissandartmouth.com',
                ...
                foreach ($objects->oreg->wuvs->results->vehicles as $obj)
                {
                    ...
                    $car_data = array(
                       ...
                       // 'trim' => $obj->trim,
                        ...
                        'url' => "https://www.oregansnissandartmouth.com/used-cars/{$obj->year}-{$obj->vehicleMake->name}-{$obj->vehicleMakeModel->name}-"
...
add_filter('filter_oregansnissandartmouth_car_data', 'filter_oregansnissandartmouth_car_data');
add_filter("filter_oregansnissandartmouth_next_page", "filter_oregansnissandartmouth_next_page", 10, 2);


function filter_oregansnissandartmouth_car_data($car_data) {
    ...
    if ($car_data['stock_type'] == 'new') {
        ...
        $car_data['url'] = str_replace('https://', 'https://www.oregansnissandartmouth.com', $car_data['url']);
    ...
    if (isset($car_data['vin']) && $car_data['vin']) {
        ...
        $in_cookies     = '';
        $out_cookies    = '';
        $response_data = HttpPost($api_url, $post_data, $in_cookies, $out_cookies, false, true, 'application/x-www-form-urlencoded', ['origin' => 'https://oregansnissandartmouth.com']);
...
function filter_oregansnissandartmouth_next_page($next, $current_page) {
