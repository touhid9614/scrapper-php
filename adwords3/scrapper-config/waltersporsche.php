<?php
    ...
    $scrapper_configs['waltersporsche'] = array(

      
        'entry_points' => array(
            'new'       => 'https://www.porscheriverside.com/new-vehicles/',
            //'used'      => 'https://www.porscheriverside.com/used-vehicles/',
        ...
        'details_start_tag' => '<div class="vehicle list-view new-vehicle publish"',
    ...
    add_filter('filter_waltersporsche_post_data', 'filter_waltersporsche_post_data',10, 3);
    add_filter('filter_waltersporsche_data', 'filter_waltersporsche_data');
    add_filter("filter_waltersporsche_field_images", "filter_waltersporsche_field_images");
    
    $waltersporsche_nonce = '';
    
    function filter_waltersporsche_post_data($post_data, $stock_type, $data)
    {
        global $waltersporsche_nonce;
        if($post_data == '')
        {
            $post_data = "page=1";
        }
        ...
        if($nonce && isset($nonce)) { $waltersporsche_nonce=$nonce; }
        slecho("global ajax_nonce : ".$waltersporsche_nonce);
        $post_id = 4;
        ...
        return "action=im_ajax_call&perform=get_results&$post_data&show_all_filters=false&_nonce=$waltersporsche_nonce&_post_id=$post_id&_referer=$referer";
    }

    function filter_waltersporsche_data($data)
    {
        if($data)
        {
            if(isJSON($data)){
    ...
    function filter_waltersporsche_field_images($im_urls)
    {
