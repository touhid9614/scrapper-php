<?php
    ...
    $scrapper_configs['maseratiofsandiego'] = array(

        'entry_points' => array(
                'new' => 'https://www.maseratiofsandiego.com/searchnew.aspx',
                'used'=> 'https://www.maseratiofsandiego.com/searchused.aspx'
        ),
        'vdp_url_regex'       => '/\/(?:new|used)-San\+Diego-[0-9]{4}-/i',
        ...
        'data_capture_regx' => array(
            'url'              => '/class="vehicleTitle\s*margin-[^>]+>\s*<a\s*href="(?<url>[^"]+)">\s*<span[^>]+>(?<title>(?<year>[0-9]{4})\s*(?<make>[^\s]+)\s*(?<model>[^<]*)\s*[^<]*)/',
            'title'            => '/class="vehicleTitle\s*margin-[^>]+>\s*<a\s*href="(?<url>[^"]+)">\s*<span[^>]+>(?<title>(?<year>[0-9]{4})\s*(?<make>[^\s]+)\s*(?<model>[^<]*)\s*[^<]*)/',
            'year'             => '/class="vehicleTitle\s*margin-[^>]+>\s*<a\s*href="(?<url>[^"]+)">\s*<span[^>]+>(?<title>(?<year>[0-9]{4})\s*(?<make>[^\s]+)\s*(?<model>[^<]*)\s*[^<]*)/',
            'make'             => '/class="vehicleTitle\s*margin-[^>]+>\s*<a\s*href="(?<url>[^"]+)">\s*<span[^>]+>(?<title>(?<year>[0-9]{4})\s*(?<make>[^\s]+)\s*(?<model>[^<]*)\s*[^<]*)/',
     ...
     add_filter("filter_maseratiofsandiego_field_images", "filter_maseratiofsandiego_field_images");

        function filter_maseratiofsandiego_field_images($im_urls)
        {
