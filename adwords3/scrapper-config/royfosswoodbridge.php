...
$scrapper_configs["royfosswoodbridge"] = array(
    'entry_points' => array(
      //they want certified used as used cars
        //https://app.asana.com/0/1145853562025778/1174720578696539/f
    ...
    'vdp_url_regex' => '/\/inventory\/(?:new|used|certified-used|certified)-[0-9]{4}-/',
     'srp_page_regex'      => '/\/(?:new|used|certified)-vehicles/i',
    'use-proxy' => true,
    'refine' => false,
    'init_method' => 'POST',
    'next_method' => 'POST',
    'picture_selectors' => ['div.swiper-lazy'],
    ...
    'custom_data_capture' => function($url, $data) {
        ...
        foreach ($objects->results[0]->hits as $obj) {
            ...
            $car_data = array(
                ...
                'make' => $obj->make,
                'model' => $obj->model,
                'trim' => strlen($obj->trim) <20 ? $obj->trim : "",
                'body_style' => $obj->body,
                'price' => $obj->our_price,
           ...
           // https://app.asana.com/0/687248649257779/1178599850457108
            if($car_data['make']=="Cadillac"){
                continue;
            }
