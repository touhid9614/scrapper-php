...
$scrapper_configs['barbermotors'] = array(
    'entry_points' => array(
        'new' => 'http://www.barbermotors.com/VehicleSearchResults?search=new',
        'used' => 'http://www.barbermotors.com/VehicleSearchResults?search=preowned',
    ),
...
add_filter("filter_barbermotors_next_page", "filter_barbermotors_next_page", 10, 2);
add_filter("filter_barbermotors_field_price", "filter_barbermotors_field_price", 10, 3);
add_filter("filter_barbermotors_field_images", "filter_barbermotors_field_images");
 
        function filter_barbermotors_field_images($im_urls)
        {
...
function filter_barbermotors_next_page($next, $current_page) {
    slecho("Filtering Next url");
    $car_type = explode('=', $current_page);
    return urlCombine($next, "?search={$car_type[count($car_type) - 1]}");
}

function filter_barbermotors_field_price($price, $car_data, $spltd_data) {
