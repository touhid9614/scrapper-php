...
$scrapper_configs["bmwautohausca"] = array(
    "entry_points"           => array(
        'used' => 'https://www.bmwautohaus.ca/used/',
        'new'  => 'https://www.bmwautohaus.ca/new/',
    ),
    ...
    'images_regx'            => '/onerror="[^"]+".*(?:data-src|src)="(?<img_url>[^"]+)/',
);

add_filter("filter_bmwautohausca_field_price", "filter_bmwautohausca_field_price", 10, 3);
add_filter("filter_bmwautohausca_field_images", "filter_bmwautohausca_field_images");

function filter_bmwautohausca_field_price($price, $car_data, $spltd_data)
{
    $prices = [];
    ...
    if ($price && numarifyPrice($price) > 0) {
        ...
        slecho("bmwautohausca Price: $price");
    }
...
function filter_bmwautohausca_field_images($im_urls)
{
