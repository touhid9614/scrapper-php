...
$scrapper_configs["oregansinfiniti"] = array(
    'entry_points' => array(
         'new'   => 'https://oserv2.oreganscdn.com/api/vehicle-inventory-search/?search.vehicle-make-ids[]=20&search.vehicle-inventory-type-ids=1&do-search=1&search.results-offset=0&search.results-limit=50&app.referrer=https://www.oregansinfiniti.com',
          'used'  => 'https://www.oregansinfiniti.com/used-cars/?search.location.id=3&search.start-index=0&do-search=1&advanced=&system.view-type.code=ajax',

        
    ),
     'vdp_url_regex' => '/\/(?:inventory|used-cars)\//',
    'refine'=>false,
    'new' => array(
         'vdp_url_regex' => '/\/(?:inventory|used-cars)\//',
        'use-proxy' => true,
            ...
            $objects = json_decode($data);
                ...
                foreach ($objects->oreg->wuvs->results->vehicles as $obj)
                {
                    ...
                    $car_data = array(
                        ...
                        'url' => "https://www.oregansinfiniti.com/used-cars/{$obj->year}-{$obj->vehicleMake->name}-{$obj->vehicleMakeModel->name}-"
...
add_filter('filter_oregansinfiniti_car_data', 'filter_oregansinfiniti_car_data');
add_filter("filter_oregansinfiniti_next_page", "filter_oregansinfiniti_next_page", 10, 2);


function filter_oregansinfiniti_car_data($car_data) {

    if($car_data['make']!='Infiniti')
    {
        return [];
    }
    ...
    if ($car_data['stock_type'] == 'new') {
        ...
        $car_data['url'] = str_replace('https://', 'https://www.oregansinfiniti.com', $car_data['url']);
    ...
    if (isset($car_data['vin']) && $car_data['vin']) {
        ...
        $in_cookies     = '';
        $out_cookies    = '';
        $response_data = HttpPost($api_url, $post_data, $in_cookies, $out_cookies, false, true, 'application/x-www-form-urlencoded', ['origin' => 'https://www.oregansinfiniti.com']);
...
function filter_oregansinfiniti_next_page($next, $current_page) {
