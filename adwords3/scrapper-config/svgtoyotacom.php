...
$scrapper_configs["svgtoyotacom"] = array( 
	"entry_points" => array(
	    'new' => 'https://www.svgtoyota.com/new-vehicles/',
        'used' => 'https://www.svgtoyota.com/used-vehicles/'
    ),
...
add_filter('filter_svgtoyotacom_post_data', 'filter_svgtoyotacom_post_data', 10, 3);
add_filter('filter_svgtoyotacom_data', 'filter_svgtoyotacom_data');

$svgtoyotacom_nonce = '';

function filter_svgtoyotacom_post_data($post_data, $stock_type, $data) {
    global $svgtoyotacom_nonce;
    if ($post_data == '') {
        $post_data = "page=1";
    }
    ...
    if ($nonce && isset($nonce)) {
        $svgtoyotacom_nonce = $nonce;
    }
    slecho("global ajax_nonce : " . $svgtoyotacom_nonce);
    $post_id = 4;
    ...
    if ($stock_type == 'used') {
     ...
     return "action=im_ajax_call&perform=get_results&$post_data&_nonce=$svgtoyotacom_nonce&_post_id=$post_id&_referer=$referer";           
}

function filter_svgtoyotacom_data($data) {
    if ($data) {
        if (isJSON($data)) {
...
add_filter("filter_svgtoyotacom_field_price", "filter_svgtoyotacom_field_price", 10, 3);

function filter_svgtoyotacom_field_price($price, $car_data, $spltd_data) {
    $prices = [];
