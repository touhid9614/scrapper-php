...
$scrapper_configs["freedombg"] = array(
    'entry_points' => array(
         'new' => 'https://www.freedombg.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1',
        'used' => 'https://www.freedombg.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
    ...
    'custom_data_capture' => function ($url, $data) {
        ...
        foreach ($objects->pageInfo->trackingData as $obj) {
            ...
            $car_data = array(
                ...
                'price' => !empty($obj->salePrice) ? $obj->salePrice : $obj->askingPrice,
                'trim' => $obj->trim,
                ...
                'url' => "https://www.freedombg.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}" . ".htm",
            );
                /*
...
add_filter("filter_freedombg_next_page", "filter_freedombg_next_page", 10, 2);

function filter_freedombg_next_page($next, $current_page) {
...
add_filter("filter_freedombg_field_images", "filter_freedombg_field_images");

function filter_freedombg_field_images($im_urls) {
    if (count($im_urls) < 2) {
        return [];
    }
