<?php
    ...
    $scrapper_configs['lexusofarlington'] = array(
         'entry_points' => array(
        'used' => 'https://www.lexusofarlington.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
        'new' => 'https://www.lexusofarlington.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1'
    ),
        ...
        'custom_data_capture' => function($url, $data) {
            ...
            foreach ($objects->pageInfo->trackingData as $obj) {
                ...
                $car_data = array(
                    ...
                    'trim' => $obj->trim,
                    'msrp' =>  explode('.',$obj->pricing->msrp)[0],
                   ...
                   // 'images'            => array_merge($obj->photos->user, $obj->photos->stock),    
                    'url' => "https://www.lexusofarlington.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
 ...
 add_filter("filter_lexusofarlington_next_page", "filter_lexusofarlington_next_page", 10, 2);       
 function filter_lexusofarlington_next_page($next, $current_page) {
...
add_filter('filter_for_fb_lexusofarlington', 'filter_for_fb_lexusofarlington', 10, 2);

function filter_for_fb_lexusofarlington($car_data, $feed_type)
{
