...
$scrapper_configs["victorychryslerdodgejeepnet"] = array( 
	"entry_points" => array(
        'new' => 'https://www.victorychryslerdodgejeep.net/searchnew.aspx',
        'used' => 'https://www.victorychryslerdodgejeep.net/searchused.aspx'
    ),
    "refine"=>false,
    ...
    'data_capture_regx' => array(
        ...
        'price' => '/Internet Price:\s*<\/span><[^>]+>(?<price>[^<]+)/',
        'vin' => '/<strong>VIN\s*#:\s*<\/strong>[^>]+>(?<vin>[^<]+)/',
    ),
    'data_capture_regx_full' => array(
    ),
    'next_page_regx' => '/<a href="(?<next>[^"]+)"[^>]+>\s*Next/',
    'images_regx' => '/<img class=\'img-responsive margin-auto\' src="(?<img_url>[^\?]+)\?[^"]+"\s*alt=/',
);


add_filter('filter_victorychryslerdodgejeepnet_field_price', 'filter_victorychryslerdodgejeepnet_field_price', 10, 3);

function filter_victorychryslerdodgejeepnet_field_price($price, $car_data, $spltd_data) {
    $prices = [];
