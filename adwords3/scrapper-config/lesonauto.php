<?php
    ...
    $scrapper_configs['lesonauto'] = array(
        'entry_points' => array(
            'new'    => 'https://www.lesonauto.com/new-vehicles/',
            'used'   => 'https://www.lesonauto.com/used-vehicles/'
        ),
        'use-proxy' => true,
        ...
        'data_capture_regx' => array(
            ...
            'price'         => '/class="price-block our-price real-price"\s*>[\s\S]*?<span class="price">\s*(?<price>\$[0-9,]+)/',
    ...
    add_filter('filter_lesonauto_post_data', 'filter_lesonauto_post_data', 10, 3);
    add_filter('filter_lesonauto_data', 'filter_lesonauto_data');
    
 
    $lesonauto_nonce = '';
    
    function filter_lesonauto_post_data($post_data, $stock_type, $data)
    {
        global $lesonauto_nonce;
        if($post_data == '')
        {
            $post_data = "page=1";
        }
        ...
        if($nonce && isset($nonce)) { $lesonauto_nonce=$nonce; }
        slecho("global ajax_nonce : ".$lesonauto_nonce);
        $post_id = 4;
        $referer = '/new-vehicles/';
            
        if($stock_type == 'used') {
            $post_id = 5;
            $referer = '/used-vehicles-harvey-la/';
        }
                
   
        return "action=im_ajax_call&perform=get_results&$post_data&_nonce=$lesonauto_nonce&_post_id=$post_id&_referer=$referer";
    }

    function filter_lesonauto_data($data)
    {
        global $lesonauto_nonce;
        if($data)
        {
