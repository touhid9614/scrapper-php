...
$scrapper_configs["allwayschevycom"] = array( 
     'entry_points' => array(
        'used' => 'https://www.allwayschevy.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
        'new' => 'https://www.allwayschevy.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1'
    ),
            ...
            foreach ($objects->pageInfo->trackingData as $obj) {
                ...
                $car_data = array(
                   ...
                   // 'price' => $obj->pricing->finalPrice,
                    ...
                    'url' => "https://www.allwayschevy.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
 ...
 add_filter("filter_allwayschevycom_next_page", "filter_allwayschevycom_next_page", 10, 2);       
 function filter_allwayschevycom_next_page($next, $current_page) {
...
add_filter("filter_allwayschevycom_field_images", "filter_allwayschevycom_field_images");
    
    function filter_allwayschevycom_field_images($im_urls)
    {
       if(count($im_urls)<10)
            {
            return [];
