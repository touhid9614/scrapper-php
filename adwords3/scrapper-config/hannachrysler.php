<?php
    ...
    $scrapper_configs['hannachrysler'] = array(
        'entry_points' => array(
            'all'   => 'http://www.hannachrysler.com/vehicles/',     
        ),
        'vdp_url_regex'     => '/\/vehicles\/[0-9]{4}\//i',
        'ty_url_regex'      => '/\/thank-you\//i',
        //'requird_params'    =>['saleclass'],
        'use-proxy' => true,
        'picture_selectors' => ['.fotorama__nav--thumbs .fotorama__nav__frame'],
        ...
        'custom_data_capture' => function($url, $resp){
            ...
            $sale_price_key = array_search('sale_price', $keys); //17
            ...
            foreach($inventory as $obj)
            {
                $car_data = array(
                    ...
                    'url'               => "http://hannachrysler.com/vehicles/{$obj[$year_key]}/{$obj[$make_key]}/{$obj[$model_key]}/Hanna/AB/{$obj[$vin_key]}/",//?saleclass={$obj[$stock_type_key]}",
                    'exterior_color'    => $obj[$exterior_color_key],
                    //'interior_color'    => $obj->interior_color
                );
                  
                $temp_data = HttpGet(str_replace(" ","%20",$car_data['url']));
                
                $images_regex   = '/<img src=\'(?<img_url>http:\/\/images\.dealertrend\.com\/[^\']+)/';
