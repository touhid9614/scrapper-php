...
$scrapper_configs["faulknertoyotatrevosecom"] = array(
    "entry_points" => array(
        'new' => 'https://www.faulknertoyotatrevose.com/new-vehicles/',
        'used' => 'https://www.faulknertoyotatrevose.com/used-vehicles/'
    ),
    ...
    'images_regx' => '/class="swiper-lazy" data-src="(?<img_url>[^"]+)/'
);

add_filter('filter_faulknertoyotatrevosecom_post_data', 'filter_faulknertoyotatrevosecom_post_data', 10, 3);
add_filter('filter_faulknertoyotatrevosecom_data', 'filter_faulknertoyotatrevosecom_data');


$faulknertoyotatrevosecom_nonce = '';

function filter_faulknertoyotatrevosecom_post_data($post_data, $stock_type, $data) {
    global $faulknertoyotatrevosecom_nonce;
    if ($post_data == '') {
        $post_data = "page=1";
    }
    ...
    if ($nonce && isset($nonce)) {
        $faulknertoyotatrevosecom_nonce = $nonce;
    }
    slecho("global ajax_nonce : " . $faulknertoyotatrevosecom_nonce);
    $post_id = 6;
    ...
    return "action=im_ajax_call&perform=get_results&$post_data&_nonce=$faulknertoyotatrevosecom_nonce&_post_id=$post_id&_referer=$referer";

}

function filter_faulknertoyotatrevosecom_data($data) {
    if ($data) {
        if (isJSON($data)) {
...
add_filter("filter_faulknertoyotatrevosecom_next_page", "filter_faulknertoyotatrevosecom_next_page", 10, 2);

function filter_faulknertoyotatrevosecom_next_page($next_page_regex) {
    slecho("Filtering Next url");
    return str_replace('?', '', $next_page_regex);
}


add_filter("filter_faulknertoyotatrevosecom_field_price", "filter_faulknertoyotatrevosecom_field_price", 10, 3);

function filter_faulknertoyotatrevosecom_field_price($price, $car_data, $spltd_data) {
    $prices = [];
