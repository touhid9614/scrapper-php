...
$scrapper_configs["martinchrysler"] = array(
   'entry_points' => array(
            'new'    => 'https://www.martinchrysler.net/new-vehicles/',
            'used'   => 'https://www.martinchrysler.net/used-vehicles/'
        ),
         //'use-proxy' => true,
        'proxy-area'        => 'FL',
    ...
    add_filter('filter_martinchrysler_post_data', 'filter_martinchrysler_post_data', 10, 3);
    add_filter('filter_martinchrysler_data', 'filter_martinchrysler_data');
    
    
    $martinchrysler_nonce = '';
            
    function filter_martinchrysler_post_data($post_data, $stock_type, $data)
    {
        global $martinchrysler_nonce;
        if($post_data == '')
        {
            $post_data = "page=1";
        }
        ...
        if($nonce && isset($nonce)) { $martinchrysler_nonce=$nonce; }
        slecho("global ajax_nonce : ".$martinchrysler_nonce);
        $post_id = 4;
        ...
        return "action=im_ajax_call&perform=get_results&$post_data&_nonce=$martinchrysler_nonce&_post_id=$post_id&_referer=$referer";
    }

    function filter_martinchrysler_data($data)
    {
        if($data)
        {
            if(isJSON($data)){
     ...
     add_filter("filter_martinchrysler_field_price", "filter_martinchrysler_field_price", 10, 3);
   
    function filter_martinchrysler_field_price($price,$car_data, $spltd_data)
    {
        $prices = [];
