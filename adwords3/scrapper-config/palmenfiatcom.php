...
$scrapper_configs["palmenfiatcom"] = array( 
	'entry_points' => array(
            'used' => 'https://www.palmenfiat.com/used-vehicles-kenosha-wi?limit=300',
        'new' => 'https://www.palmenfiat.com/new-vehicles-kenosha-wi?limit=300',
    ...
    'data_capture_regx' => array(
       ...
       // 'title' => '/class="vehicle-title ">\s*(?<title>(?<year>[0-9]{4})\s*(?<make>[^\s]+)\s*(?<model>[^\n]+))/',
        ...
        'price' => '/Retail Price\s*[^>]+>[^>]+>\s*(?<price>\$[0-9,]+)/',
  ...
  add_filter("filter_palmenfiatcom_field_price", "filter_palmenfiatcom_field_price", 10, 3);
     function filter_palmenfiatcom_field_price($price,$car_data, $spltd_data)
    {
        $prices = [];
        ...
        $msrp_regex       =  '/Palmen Price[^>]+>\s*[^>]+>\s*(?<price>\$[0-9,]+)/';
