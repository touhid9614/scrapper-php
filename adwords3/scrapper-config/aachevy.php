...
$scrapper_configs["aachevy"] = array(
    'entry_points' => array(
        'new' => 'https://www.aachevy.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1',
        'used' => 'https://www.aachevy.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
    ),
            ...
            foreach ($objects->pageInfo->trackingData as $obj) {
                ...
                $car_data = array(
                   ...
                   // 'images'            => array_merge($obj->photos->user, $obj->photos->stock),    
                    'url' => "https://www.aachevy.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
 ...
 add_filter("filter_aachevy_next_page", "filter_aachevy_next_page", 10, 2);       
 function filter_aachevy_next_page($next, $current_page) {
...
add_filter("filter_aachevy_field_images", "filter_aachevy_field_images");
    
    function filter_aachevy_field_images($im_urls)
    {
       if(count($im_urls)<2)
            {
            return [];
...
// add_filter("filter_for_fb_aachevy", "filter_for_fb_aachevy");
    
// function filter_for_fb_aachevy($car)
// {
//     $car['vin'] = [];
