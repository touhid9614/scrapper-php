<?php
    ...
    $scrapper_configs['schaumburglincoln'] = array(
        'entry_points' => array(
            'new' => 'https://www.schaumburglincoln.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1',
            
             'used' => 'https://www.schaumburglincoln.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
            ...
            foreach ($objects->pageInfo->trackingData as $obj) {
                ...
                $car_data = array(
                   ...
                   // 'images'            => array_merge($obj->photos->user, $obj->photos->stock),    
                    // 'url' => "https://www.schaumburglincoln.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
                    'url' 		 => "https://www.schaumburglincoln.com{$obj->link}",          
                );
 ...
 add_filter("filter_schaumburglincoln_next_page", "filter_schaumburglincoln_next_page", 10, 2);       
 function filter_schaumburglincoln_next_page($next, $current_page) {
...
add_filter("filter_schaumburglincoln_field_images", "filter_schaumburglincoln_field_images");

function filter_schaumburglincoln_field_images($im_urls) 
