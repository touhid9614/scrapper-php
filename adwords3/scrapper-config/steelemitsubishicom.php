...
$scrapper_configs["steelemitsubishicom"] = array( 
	'entry_points' => array(
          'used' => 'https://www.steelemitsubishi.com/used-inventory/index.htm',
        'new' => 'https://www.steelemitsubishi.com/new-inventory/index.htm',
   ...
   'next_page_regx' => '/<a class="btn btn-link btn-xs"\s*href="(?<next>[^"]+)"\s*rel="next"/',
    'images_regx' => '/"src"[^"]+"(?<img_url>[^"]+)"[^"]+"thumbnail/',
    'images_fallback_regx' => '/<meta name="og:image" content="(?<img_url>[^"]+)"/'
);
add_filter("filter_steelemitsubishicom_field_price", "filter_steelemitsubishicom_field_price", 10, 3);
add_filter("filter_steelemitsubishicom_field_images", "filter_steelemitsubishicom_field_images");

function filter_steelemitsubishicom_field_images($im_urls) {
    $retval = [];

    foreach ($im_urls as $img) {

        $retval[] = str_replace(["|", "%20"], ["%7C"," "], $img);
    }

    return $retval;
}

function filter_steelemitsubishicom_field_price($price, $car_data, $spltd_data) {
    $prices = [];
