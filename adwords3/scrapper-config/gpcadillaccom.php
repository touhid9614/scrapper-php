...
$scrapper_configs["gpcadillaccom"] = array( 
	'entry_points' => array(
 ...
 foreach($objects->results[0]->hits as $obj)
        {
            ...
            $regex       =  '/Please call for price/';
            
            if (preg_match($regex, $car_data['price'], $matches)) {
                 slecho("bbbbbbbb");
                continue;
            }
...
add_filter("filter_gpcadillaccom_field_images", "filter_gpcadillaccom_field_images",10,2);
function filter_gpcadillaccom_field_images($im_urls, $car_data) {
    $retval = array();
