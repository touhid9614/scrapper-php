...
$scrapper_configs["canbyfordcom"] = array(
    'entry_points' => array(
        'new' => 'https://search-a5-jazel-tango.jazel-qa.com/api/passthroughsrp?q=(account_id%3A917%20AND%20(vehicle_filter_ids%3A2)%20AND%20((((make_sidebar%3A%22Ford%22)))))&q.parser=lucene&size=200&sort=has_price%20desc%2Chas_image%20desc%2Cdate_in_stock%20asc&start=0',
        'used' => 'https://search-a5-jazel-tango.jazel-qa.com/api/passthroughsrp?q=(account_id%3A917%20AND%20(vehicle_filter_ids%3A3))&q.parser=lucene&size=100&sort=has_price%20desc%2Chas_image%20desc%2Cdate_in_stock%20asc&start=0'
    ),
    ...
    'custom_data_capture' => function($url, $data) {
        ...
        foreach ($objects->hits->hit as $obj) {
            ...
            $car_data = array(
                ...
                //'msrp'              => $obj->msrp,
                'url' => "https://www.canbyford.com/inventory/" . ($obj->fields->used ? "used" : "new") . "-vehicles/vehicle/" . $obj->fields->vin .
                "/{$obj->fields->condition}-{$obj->fields->year}-{$obj->fields->make}-{$obj->fields->model}-" .
...
add_filter("filter_canbyfordcom_field_images", "filter_canbyfordcom_field_images", 10, 2);

function filter_canbyfordcom_field_images($im_urls, $car_data) {
    $retval = array();
    // slecho(implode('|', $im_urls));

    $ignore = "https://www.canbyford.com/inventory/" . $car_data['stock_type'] . "-vehicles/vehicle/" . $car_data['vin'] . "/";
