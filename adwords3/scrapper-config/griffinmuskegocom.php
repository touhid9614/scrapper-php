...
$scrapper_configs["griffinmuskegocom"] = array( 
	'entry_points' => array(     
            'used' => 'https://www.griffinmuskego.com/searchused.aspx',
              'new'  => 'https://www.griffinmuskego.com/searchnew.aspx',
        ),
...
add_filter("filter_griffinmuskegocom_field_price", "filter_griffinmuskegocom_field_price", 10, 3);
        function filter_griffinmuskegocom_field_price($price,$car_data, $spltd_data)
        {
            $prices = [];
            ...
            if($price && numarifyPrice($price) > 0) {
                ...
                slecho("griffinmuskegocom Price: $price");
            }
           $retail_regex =  '/Internet Price\s*[^>]+>[^>]+>(?<price>\$[0-9,]+)/';
            $msrp_regex =  '/Price:\s*[^>]+>[^>]+>(?<price>\$[0-9,]+)/';
            $internet_regex =  '/Market Price:\s*[^>]+>[^>]+>(?<price>\$[0-9,]+)/';
            $matches = [];
