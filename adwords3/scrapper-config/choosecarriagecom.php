...
$scrapper_configs["choosecarriagecom"] = array( 
	 'entry_points'        => array(
    ...
    'custom_data_capture' => function ($url, $data) {
        ...
        foreach ($objects->results[0]->hits as $obj) {
           ...
           $temp_data = HttpGet($car_data['url']);
            ...
            $response_data = HttpGet($car_data['url']);
            $regex = '/Retail Price:[^>]+>\s*[^>]+>(?<msrp>[^<]+)/';
            $matches = [];
            if (preg_match($regex, $response_data, $matches)) {

                $car_data['msrp'] = $matches['msrp'];
            }  
...
//add_filter("filter_choosecarriagecom_field_images", "filter_choosecarriagecom_field_images");
//function filter_choosecarriagecom_field_images($im_urls) {
//    if (count($im_urls) <6) {
//        return array();
//    }
//    return $im_urls;
