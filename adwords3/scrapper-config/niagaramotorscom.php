...
$scrapper_configs["niagaramotorscom"] = array(
	'entry_points' => array(
            'used'   => 'https://www.niagaramotors.com/en/used-inventory/api/listing?limit=500', 
            'new'    => 'https://www.niagaramotors.com/en/new-catalog',  
        ),
        ...
        'used' => array(
                   ...
                   $to_return = array();
                       ...
                       $car_data = array(
                           ...
                           'url'               => "https://www.niagaramotors.com/en/used-inventory/" . $obj->make->name . '/' . $obj->model->name . '/' . $obj->year . '/' . $obj->year . '-' . $obj->make->name . '-' . $obj->model->name . '-id' . $obj->vehicleId,
        ...
        'custom_data_capture' => function($url, $resp) {
            ...
            foreach ($inventory as $url) {
                ...
                $data_capture_regx_full = array(
                    ...
                    'year'       => '/year:\'(?<year>[^\']+)/',
                    'trim'       => '/data-desired-trim="(?<trim>[^"]+)"\s*/',
                    'price'      => '/displayedPrice:\s*\'(?<price>[^\']+)/',
...
function niagaramotorscom_get_newInventory($url, $temp) {
    global $proxy_list;
    ...
    foreach ($spltd as $data) {
        ...
        if (preg_match($url_regex, $data, $matches)) {
            $vdp_url = 'https://www.niagaramotors.com' . $matches['url'];
        }
    ...
    $ttl_inventory = array_map(function($url) {
        return 'https://www.niagaramotors.com/' . $url;
    }
            , $ttl_inventory);
