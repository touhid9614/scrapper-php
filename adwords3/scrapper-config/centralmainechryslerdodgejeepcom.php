...
$scrapper_configs["centralmainechryslerdodgejeepcom"] = array( 
	"entry_points" => array(
	    'new' => 'https://www.centralmainechryslerdodgejeep.com/new-vehicles/',
        'used' => 'https://www.centralmainechryslerdodgejeep.com/used-vehicles/'
    ),
  //  'use-proxy' => true,
   ...
   // 'details_start_tag' => '<table class="results_table">',
    ...
    'data_capture_regx' => array(
        ...
        'price' => '/Sale Price[^>]+>\s*[^>]+>\s*(?<price>\$[0-9,]+)/',
...
add_filter('filter_centralmainechryslerdodgejeepcom_post_data', 'filter_centralmainechryslerdodgejeepcom_post_data', 10, 3);
add_filter('filter_centralmainechryslerdodgejeepcom_data', 'filter_centralmainechryslerdodgejeepcom_data');

$centralmainechryslerdodgejeepcom_nonce = '';

function filter_centralmainechryslerdodgejeepcom_post_data($post_data, $stock_type, $data) {
    global $centralmainechryslerdodgejeepcom_nonce;
    if ($post_data == '') {
        $post_data = "page=1";
    }
    ...
    if ($nonce && isset($nonce)) {
        $centralmainechryslerdodgejeepcom_nonce = $nonce;
    }
    slecho("global ajax_nonce : " . $centralmainechryslerdodgejeepcom_nonce);
    $post_id = 5;
    ...
    if ($stock_type == 'used') {
     ...
     return "action=im_ajax_call&perform=get_results&$post_data&_nonce=$centralmainechryslerdodgejeepcom_nonce&_post_id=$post_id&_referer=$referer";           
}

function filter_centralmainechryslerdodgejeepcom_data($data) {
    if ($data) {
        if (isJSON($data)) {
...
add_filter("filter_centralmainechryslerdodgejeepcom_field_price", "filter_centralmainechryslerdodgejeepcom_field_price", 10, 3);

function filter_centralmainechryslerdodgejeepcom_field_price($price, $car_data, $spltd_data) {
    $prices = [];
    ...
    $msrp_regex = '/<span class="price-label">\s*Price\s*[^>]+>[^>]+>\s*(?<price>[^<]+)/';
