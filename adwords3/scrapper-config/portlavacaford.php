<?php
    ...
    $scrapper_configs['portlavacaford'] = array(
        'entry_points' => array(
            'new'    => 'https://www.portlavacaford.com/new-vehicles/',
            'used'   => 'https://www.portlavacaford.com/used-vehicles/'
        ),
        'use-proxy' => true,
        ...
        'data_capture_regx_full' => array(
            'msrp'          => '/MSRP\s*<\/span>\s*<span class="ctabox-price">\s*(?<msrp>\$[0-9,]+)/',
    ...
    add_filter('filter_portlavacaford_post_data', 'filter_portlavacaford_post_data',10, 3);
    add_filter('filter_portlavacaford_data', 'filter_portlavacaford_data');
    add_filter("filter_portlavacaford_field_images", "filter_portlavacaford_field_images");

    
    $portlavacaford_nonce = '';
    
    function filter_portlavacaford_post_data($post_data, $stock_type, $data)
    {
        global $portlavacaford_nonce;
        if($post_data == '')
        {
            $post_data = "page=1";
        }
        ...
        if($nonce && isset($nonce)) { $portlavacaford_nonce=$nonce; }
        slecho("global ajax_nonce : ".$portlavacaford_nonce);
        $post_id = 4;
        ...
        return "action=im_ajax_call&perform=get_results&$post_data&show_all_filters=false&_nonce=$portlavacaford_nonce&_post_id=$post_id&_referer=$referer";
    }

    function filter_portlavacaford_data($data)
    {
        if($data)
        {
            if(isJSON($data)){
    ...
    function filter_portlavacaford_field_images($im_urls)
    {
