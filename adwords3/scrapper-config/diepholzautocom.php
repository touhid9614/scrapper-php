...
$scrapper_configs["diepholzautocom"] = array( 
	'entry_points' => array(
        'used' => 'https://www.diepholzauto.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
        'new' => 'https://www.diepholzauto.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1'
    ),
        ...
        'custom_data_capture' => function($url, $data) {
            ...
            foreach ($objects->pageInfo->trackingData as $obj) {
                ...
                $car_data = array(
					...
					'url'	=> "https://www.diepholzauto.com{$obj->link}",
                   // 'images'            => array_merge($obj->photos->user, $obj->photos->stock),    
                    // 'url' => "https://www.diepholzauto.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
 ...
 add_filter("filter_diepholzautocom_next_page", "filter_diepholzautocom_next_page", 10, 2);       
 function filter_diepholzautocom_next_page($next, $current_page) {
