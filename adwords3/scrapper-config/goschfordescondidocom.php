...
$scrapper_configs["goschfordescondidocom"] = array(
    'entry_points' => array(
        'used' => 'https://www.goschfordescondido.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',
        'new' => 'https://www.goschfordescondido.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1'
    ),
    ...
    'custom_data_capture' => function($url, $data) {
        ...
        foreach ($objects->pageInfo->trackingData as $obj) {
            ...
            $car_data = array(
                ...
                'url' => "https://www.goschfordescondido.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
...
add_filter("filter_goschfordescondidocom_next_page", "filter_goschfordescondidocom_next_page", 10, 2);
function filter_goschfordescondidocom_next_page($next, $current_page) {
...
add_filter('filter_goschfordescondidocom_field_price', 'filter_goschfordescondidocom_field_price', 10, 3);

function filter_goschfordescondidocom_field_price($price, $car_data, $spltd_data) {
    $prices = [];
    ...
    $price_regex = '/Final Price[^>]+>:<\/span><\/span>[^>]+>(?<price>\$[0-9,]+)/';
