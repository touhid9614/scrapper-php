...
$scrapper_configs["fortwaynekia"] = array(
  'entry_points' => array(

        'certified' => 'https://www.fortwaynekia.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_CERTIFIED_USED:inventory-data-bus1/getInventory?start=0&page=1',

        'used' => 'https://www.fortwaynekia.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_USED:inventory-data-bus1/getInventory?start=0&page=1',

        'new' => 'https://www.fortwaynekia.com/apis/widget/INVENTORY_LISTING_DEFAULT_AUTO_NEW:inventory-data-bus1/getInventory?start=0&page=1',
    ),
            ...
            foreach ($objects->pageInfo->trackingData as $obj) {
                ...
                $car_data = array(
                   ...
                   // 'images'            => array_merge($obj->photos->user, $obj->photos->stock),    
                    // 'url' => "https://www.fortwaynekia.com/{$obj->newOrUsed}/{$obj->make}/{$obj->modelYear}-{$obj->make}-{$obj->model}-{$obj->uuid}".".htm",
                    'url'          => "https://www.fortwaynekia.com{$obj->link}",
 ...
 add_filter("filter_fortwaynekia_next_page", "filter_fortwaynekia_next_page", 10, 2);       
 function filter_fortwaynekia_next_page($next, $current_page) {
...
add_filter('filter_for_fb_fortwaynekia', 'filter_for_fb_fortwaynekia', 10, 2);

function filter_for_fb_fortwaynekia($car_data, $feed_type)
{
    
        if(substr_count($car_data['all_images'], "|") < 2){
            return null;
        }
