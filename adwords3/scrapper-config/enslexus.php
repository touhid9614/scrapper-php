...
$scrapper_configs['enslexus'] = array(
    'entry_points'        => array(
    ...
    'custom_data_capture' => function ($url, $data) {
        ...
        foreach ($objects->results[0]->hits as $obj) {
            ...
            ////removing cars that VDP url redirecting to SRP.
            $custom_regex = '/class="vdp-title__vin-stock">[^>]+>[^>]+>[^>]+>[^>]+>(?<custom>[^<]+)/';
            $another_matches = [];
            if(preg_match($custom_regex, $response_data, $another_matches)){
                $car_data['custom'] = $another_matches['custom'];
            }
...
//add_filter("filter_enslexus_field_images", "filter_enslexus_field_images");
//function filter_enslexus_field_images($im_urls)
//{
//    // $retval = [];
//    if (count($im_urls) < 3) {return [];} else {return $im_urls;}
//}

add_filter('filter_enslexus_car_data', 'filter_enslexus_car_data');

function filter_enslexus_car_data($car_data) {
    if($car_data['custom'] == ""){
        $car_data = [];
        //removing cars that VDP url redirecting to SRP.
    }
