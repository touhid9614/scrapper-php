...
$scrapper_configs["oreganskiadartmouthcom"] = array(
    'entry_points' => array(
        'new' => 'https://oserv2.oreganscdn.com/api/vehicle-inventory-search/?do-search=1&search.results-offset=0&search.results-limit=50&&app.referrer=https://www.oreganskiadartmouth.com',
        'used' => 'https://www.oreganskiadartmouth.com/inventory/pre-owned/?search.start-index=0&search=1&do-search=1&advanced=0&system.view-type.code=ajax',
            ...
            foreach ($objects->oreg->wuvs->results->vehicles as $obj) {
                ...
                $car_data = array(
                   ...
                   // 'trim' => $obj->trim,
                    ...
                    'url' => "https://www.oreganskiadartmouth.com/inventory/pre-owned/{$obj->year}-{$obj->vehicleMake->name}-{$obj->vehicleMakeModel->name}-"
...
add_filter('filter_oreganskiadartmouthcom_car_data', 'filter_oreganskiadartmouthcom_car_data');
add_filter("filter_oreganskiadartmouthcom_next_page", "filter_oreganskiadartmouthcom_next_page", 10, 2);

function filter_oreganskiadartmouthcom_car_data($car_data) {
    ...
    if ($car_data['stock_type'] == 'new') {
        $car_data['url']= "https://www.oreganskiadartmouth.com/inventory/new/" .  $car_data['url'];
        $car_data['url'] = preg_replace('/http(s)?:.*(?=http)/', '', $car_data['url'], -1);
        $car_data['url'] = str_replace('https://', 'https://www.oreganskiadartmouth.com', $car_data['url']);
    ...
    if (isset($car_data['vin']) && $car_data['vin']) {
        ...
        $in_cookies = '';
        $out_cookies = '';
        $response_data = HttpPost($api_url, $post_data, $in_cookies, $out_cookies, false, true, 'application/x-www-form-urlencoded', ['origin' => 'https://www.oreganskiadartmouth.com']);
...
function filter_oreganskiadartmouthcom_next_page($next, $current_page) {
