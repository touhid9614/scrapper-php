...
$scrapper_configs['georgianchevrolet'] = array(
    'entry_points' => array(
    ...
    'custom_data_capture' => function($url, $data) {
        ...
        foreach ($objects->results[0]->hits as $obj) {
            ...
            $sold_car_images_regex = '/<img class="custom_badge" src="(?<img_url>[^"]+)" alt="Sold Badge">/';
            $matches = [];
            
            if (preg_match_all($sold_car_images_regex, $response_data, $matches)) {
                continue;
            }
...
function filter_georgianchevrolet_field_images($im_urls) {
    if (count($im_urls) < 5) {
        return array();
    }
    return $im_urls;
