...
$scrapper_configs["langleytoyotacom"] = array( 
   'entry_points' => array(
        ...
        'srp_page_regex'         => '/(?:used|new)-vehicles\//',
           ...
           if(preg_match_all($images_regex, $temp_data, $matches))
                {
                 ...
                 $regex2         = '/<meta property="og:description" content="(?<description>[^"]+)/';
            $matches       = [];

            if (preg_match($regex2, $response_data, $matches)) {
...
add_filter("filter_langleytoyotacom_field_images", "filter_langleytoyotacom_field_images",10,2);
function filter_langleytoyotacom_field_images($im_urls, $car_data) {
    $retval = array();
