...
/* $exec_str = "ps aux | grep -i php | grep bing-v2.php | grep hondasbynelson | grep -v grep | awk '{print $2, $13}'";
$bing_worker  = array_filter(explode(" ", exec($exec_str)));
if (count($bing_worker)) {
    var_dump($bing_worker);
    exit();
}  */
                ...
                <div class="inner-wrapper">
                    <section role="main" class="content-body">
                        <header class="page-header" style="margin: 40px 0;">
                            <h1 class="panel-title" style="font-size: 20px; color: #7EC0EE; font-weight: bold"> Bing Ads Control Panel </h1>
                        </header>
                            ...
                            <div class="col-lg-12">
                                ...
                                if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                                    if (isset($_POST['killme'])) {
                                        ...
                                        if (`ps aux |grep -v grep | grep $pid | grep bing-v2.php | wc -l ` == 1) {
                                            `kill $pid`;
                                        }
                                    } else if (isset($_POST['killall'])) {
                                        if ($_POST['killall']) {
                                            exec("ps aux |  grep -i php | grep bing-v2.php | grep -v grep | awk '{print $2}' | xargs kill");
                                        }
                                        ...
                                        if (($command != '') && ($dealership != '')) {
                                            $launch_str = $php_binary . ' '
                                                . escapeshellarg($root_dir . '/services/bing-v2.php') . ' '
                                ...
                                $worker_list = explode("\n", `ps aux |  grep -i php | grep bing-v2.php | grep -v grep | awk '{print $2, $13, $14, $3, $10, $8}'`);
                                echo '<br>';
