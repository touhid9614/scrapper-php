...
foreach ($dealerships as $cron_name => $id) {
    ...
    $log_file_path = dirname(__DIR__) . '/adwords3/caches/report-log/' . $cron_name . '-session.txt';
            ...
            foreach ($year as $y) {
                foreach ($month as $m) {
                    ...
                    if ($date_now > $sdate) {
                        ...
                        $report = $analytics->GetReport($profileId, date($sdate), date($edate), array('ga:users', 'ga:newUsers', 'ga:sessions', 'ga:bounceRate', 'ga:pageviewsPerSession', 'ga:avgSessionDuration'), array('ga:channelGrouping'));
