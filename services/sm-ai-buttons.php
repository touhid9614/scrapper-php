...
require_once $base_dir . '/event-analytics/quick-sales/include/functions.php';
require_once $adwords_dir . 'uuid.php';
require_once $tag_dir . 'init-db.php';
...
/*
 * If any email_template configuration found for particular dealer, then we will merge that with default template
 */
$config_templates = isset($cron_config['email_templates']) ? $cron_config['email_templates'] : [];
$email_templates  = array_replace_recursive($email_templates, $config_templates);
...
$algorithm   = filter_input(INPUT_POST, 'algorithm');
...
switch ($action) {
	case 'button_viewed':
		if ($algorithm == 'default') {
			...
			// file_put_contents('ai-tracker-post.txt', var_export($_POST, true) . "\n\n", FILE_APPEND);
			do_action("smart_button_combination_new_viewed", $dealership, $combination, $algorithm, $cache_file, $stock_type);
		}
	...
	case 'clicked':
		if ($algorithm == 'default') {
			...
			do_action("smart_button_combination_new_clicked", $dealership, $combination, $algorithm, $cache_file, $stock_type);
		}
		...
		if ($car) {
			...
			$url          = $car['url'];
			$day          = date('Y-m-d');
			$query_str    = "SELECT id, clicks FROM dealerships_button_tracker WHERE dealership='$dealership'" . " AND stock_number='$stock_number' AND url='$url' AND day='$day'";
	...
	case 'fillup':
		...
		$redirect_url = isset($cron_config['fillup_redirect_url']) ? $cron_config['fillup_redirect_url'] : null;
		...
		if ($algorithm == 'default') {
			...
			do_action("smart_button_combination_new_fillup", $dealership, $combination, $algorithm, $cache_file, $stock_type);
		}
		...
		$last_insert_id = DbConnect::get_connection_read()->insert_id; // Is it working?
		...
		$star    = processTextTemplate($email_templates[$form]['STAR'], $template_params);
		...
		SendEmail($template_params['star_to'], $template_params['lead_from'], $subject, $star, 'text/plain');

		// SEND MAIL FOR NEW, USED IF CONFIGURED
		$status = 'init';
		...
		/*
         * If calldrip active for this dealer, then call calldrip
         */
		$dealer_info = $db_connect->get_all_dealers("`dealership` = '$dealership'");

		if ($dealer_info[$dealership]['calldrip']) {
			$calldrip_insert_data = [
				'lead_id'    => $last_insert_id,
				'dealership' => $dealership,
				'name'       => $template_params['first_name'] . " " . $template_params['last_name'],
				'interest'   => $car['year'] . " " . $car['make'] . " " . $car['model'],
				'price'      => $template_params['price'],
				'number'     => format_number($template_params['phone']),
			];

			$db_connect->callDripInsert($calldrip_insert_data);
		}
		...
		if ($car) {
			...
			$url          = $car['url'];
			$day          = date('Y-m-d');
			$query_str    = "SELECT id, fillups FROM dealerships_button_tracker WHERE dealership='$dealership' " . "AND stock_number='$stock_number' AND url='$url' AND day='$day';";
			$result       = $db_connect->query($query_str);
			...
			if (mysqli_num_rows($result)) {
				...
				$id      = $rowdata[0];
				$fillups = $rowdata[1] + 1;
				$db_connect->query("UPDATE dealerships_button_tracker SET fillups='$fillups' WHERE id='$id';");
			} else {
				// For insert
				$insert_data = [
					'dealership'   => $dealership,
		...
		echo json_encode(['message' => 'Submitted', 'success' => true, 'redirect_url' => $redirect_url]);
		break;
