...
$site_rules['autotrader'] = array(
    'match-rules'   => array(
        '/<a.+?(?=href)href="(?<url>\/(?:new|used)-inventory\/(?:index.htm)?)/',
        '/(?:www\.autotrader\.|AutoTRADER)/'
    ),
    'entry_points'  => array(
        'new'       => array(
    ...
    'scrapper'      => array(
        array('rule'  => '/<ul class="inventoryList data full/', 'name'  => 'autotrader'),
        array('rule'  => '/<ul id="fullview">/', 'name'  => 'autotradervr')
    )
);

function autotrader_dealer_address($host, $data)
{
    $regexes = array(
    ...
    $phone_regex = '/(?<phone>Sales|Service|Parts)<\/span><span class="separator">:<\/span>\s*<span class="value">(?<number>[^<]+)/';
